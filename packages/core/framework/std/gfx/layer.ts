...
import { GfxLocalElementModel } from './scene/surface/local-element-model';
import { SurfaceBlockModel } from './scene/surface/surface-model';
      ...
      this._disposable.add(
        surface.elementUpdated.subscribe(payload => {
          if (payload.props['index'] || payload.props['childIds']) {
            const element = surface.getElementById(payload.id);
            if (!element) return;
            this.update(element, payload.props);
          }
        })
      );
      this._disposable.add(
