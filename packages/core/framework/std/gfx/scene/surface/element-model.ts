...
import type { EditorHost } from '../../../dom';
import type {
  ...
  implements GfxGroupCompatibleInterface
{
  ...
  abstract children: {
    ...
    observe(callback: (...args: unknown[]) => void): void;
    set(key: string, value: unknown): unknown;
    unobserve(callback: (...args: unknown[]) => void): void;
  };
...
function watchText(
  ...
  const fn = (_: unknown, transaction?: { local?: boolean }) => {
    callback({
      props: {
        [key]: value,
      },
      oldValues: {},
      local: transaction?.local ?? true,
    });
  };
...
function isObservableText(
  value: unknown
): value is {
  observe(
    callback: (event: unknown, transaction?: { local?: boolean }) => void
  ): (() => void) | void;
} {
  return (
    value != null &&
