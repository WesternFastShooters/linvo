...
import type { BaseTextAttributes, DeltaInsert } from '@linvo-core/store';
...
import { InlineHookService } from './services/hook';
import {
  AttributeService,
  DeltaService,
  EventService,
...
import { InlineTextService } from './services/text';
import type { InlineRange, InlineTextModel } from './types';
import { nativePointToTextPoint, textPointToDomPoint } from './utils';
import { getTextNodesFromElement } from './utils/text';
...
> {
  ...
  readonly yText: InlineTextModel<TextAttributes>;
  get yTextDeltas() {
    return this.yText.toDelta() as DeltaInsert<TextAttributes>[];
  }
  get yTextLength() {
  ...
  constructor(
    yText: InlineTextModel<TextAttributes>,
    ops: {
    ...
    if (!yText.attached) {
      throw new LinvoError(
        ...
        'yText must be attached to a document'
      );
    }
  ...
  transact(fn: () => void, withoutTransact = false): void {
    this.yText.transact(fn, withoutTransact);
  }
}
