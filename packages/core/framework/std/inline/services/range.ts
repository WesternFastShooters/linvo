...
import type { VLine } from '../components/v-line';
import type { InlineEditor } from '../inline-editor';
import type { InlineRange, TextPoint } from '../types';
import { isInEmbedGap } from '../utils/embed';
import { isMaybeInlineRangeEqual } from '../utils/inline-range';
import {
  domRangeToInlineRange,
  inlineRangeToDomRange,
} from '../utils/range-conversion';
import { calculateTextLength, getTextNodesFromElement } from '../utils/text';

export class RangeService<TextAttributes extends BaseTextAttributes> {
  private _lastEndRelativePosition: unknown = null;

  private _lastStartRelativePosition: unknown = null;
  ...
  mount = () => {
    ...
    editor.disposables.add(
      effect(() => {
        ...
        if (newInlineRange) {
          this._lastStartRelativePosition = yText.createCursor(
            newInlineRange.index
          );
          this._lastEndRelativePosition = yText.createCursor(
  ...
  toInlineRange = (range: Range): InlineRange | null => {
    const { rootElement } = this.editor;
    if (!rootElement) return null;
    return domRangeToInlineRange(range, rootElement, this.editor.yTextLength);
  };
