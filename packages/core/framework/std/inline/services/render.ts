...
import type { InlineRange, InlineTextChangeEvent } from '../types';
import { deltaInsertsToChunks } from '../utils/delta-convert';

export class RenderService<TextAttributes extends BaseTextAttributes> {
  private readonly _onYTextChange = (event: InlineTextChangeEvent) => {
    ...
    if (!inlineRange || event.isLocal) return;

    const startIndex = this.editor.yText.resolveCursor(
      this.editor.lastStartRelativePosition
    );
    const endIndex = this.editor.yText.resolveCursor(
      this.editor.lastEndRelativePosition
    );
