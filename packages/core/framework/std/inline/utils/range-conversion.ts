...
import { VElement } from '../components/v-element';
import type { InlineRange } from '../types';
import { isInEmbedElement } from './embed';
import {
  nativePointToTextPoint,
  textPointToDomPoint,
} from './point-conversion';
import { calculateTextLength, getTextNodesFromElement } from './text';
...
const rangeOnlyHasAnchorHandler: Handler = ({
  ...
  return {
    ...
    length: textLength - startDomPoint.index,
  };
};
...
const rangeHasNoAnchorAndFocusHandler: Handler = ({ textLength }) => {
  return {
    index: 0,
    length: textLength,
  };
...
export function domRangeToInlineRange(
  ...
  const context = buildContext(range, rootElement, textLength);
