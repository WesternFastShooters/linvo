...
import { isPureObject, native2Y } from '../../reactive';
...
export class DocCRUD {
  ...
  constructor(
    private readonly _yBlocks: Y.Map<BlockRecord>,
    private readonly _schema: Schema
  ) {}

  private _getSiblings<T>(
    id: string,
    fn: (index: number, parent: BlockRecord) => T
  ) {
  ...
  addBlock(
    ...
    const yBlock = new Y.Map() as BlockRecord;
  ...
  deleteBlock(
    ...
    if (!parent) return;
    const yParent = this._yBlocks.get(parent) as BlockRecord;
    const yParentChildren = yParent.get('sys:children') as Y.Array<string>;
    ...
    const apply = () => {
      ...
      if (deleteChildren) {
        ...
        const deleteById = (id: string) => {
          const yBlock = this._yBlocks.get(id) as BlockRecord;
