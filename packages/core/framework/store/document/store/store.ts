...
import type { ExtensionType } from '../../extension/extension';
import {
...
import { DocIdentifier } from '../../extension/doc-source/doc';
import { Schema } from '../../schema';
import type { TransformerMiddleware } from '../../transformer/middleware';
...
import { type Query, runQuery } from './query';
import { syncBlockProps } from './utils';
...
/**
 * Core store class that manages blocks and their lifecycle in the editor store
 * @remarks
...
export class Store {
  ...
  /**
   * Get the blob storage bridge for current store.
   */
  get blobSync() {
  ...
  undo = () => {
    ...
    return this._history.undoManager.undo();
  };
  ...
  redo = () => {
    ...
    return this._history.undoManager.redo();
  };
  ...
  transact(fn: () => void, shouldTransact: boolean = this._shouldTransact) {
    const docState = this._doc.docState;
    this._history.beginTransaction(shouldTransact);
    docState.transact(
      () => {
        try {
          fn();
        } catch (e) {
          console.error(`An error occurred while Y.doc ${docState.guid} transacting:`);
          console.error(e);
        }
      },
      shouldTransact ? this.docState.clientID : null
    );
    this._history.endTransaction(shouldTransact);
  }
   ...
   * @category History
   */
  withoutTransact<T>(fn: () => T | Promise<T>): T | Promise<T> {
  ...
  /**
   * Get the {@link AbstractDocSource} instance for current store.
   */
  get docSource() {
    return this._doc.docSource;
  }

  /**
   * Get the history controller instance for current store.
   *
   * @category History
   */
  get history() {
   ...
   * @throws {LinvoError} When store is in readonly mode
   *
   * @category Block CRUD
   */
  addBlock<T extends BlockModel = BlockModel>(
    ...
    const id = blockProps.id ?? this._doc.docSource.idGenerator();
   ...
   * @throws {LinvoError} When the block is not found or schema validation fails
   *
   * @category Block CRUD
   */
  ...
  getTransformer(middlewares: TransformerMiddleware[] = []) {
    return new Transformer({
      schema: this.schema,
      blobCRUD: this.docSource.blobSync,
      docCRUD: {
        create: (id: string) => {
          if (id !== this.id) {
            throw new LinvoError(
  ...
  private _handleYEvent(event: Y.YEvent<Y.AbstractType>) {
    ...
    const origin = (event.transaction.origin ?? null) as
      | { proxy?: boolean }
      | null;
    const yEvent = event as Y.YMapEvent<BlockRecord>;
    const isLocal =
  ...
  private readonly _handleYEvents = (events: Y.YEvent<Y.AbstractType>[]) => {
