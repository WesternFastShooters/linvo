...
import type { HistoryStackItem } from '../history/history-extension';
...
export class StoreSelectionExtension extends StoreExtension {
  ...
  private readonly _itemAdded = (event: { stackItem: HistoryStackItem }) => {
    event.stackItem.meta.set('selection-state', this._selections.value);
  };

  private readonly _itemPopped = (event: { stackItem: HistoryStackItem }) => {
  ...
  clearRemote() {
    if (this._remoteSelections.peek().size === 0) {
      return;
    }

    const cleared = new Map<number, BaseSelection[]>();
    this._remoteSelections.value = cleared;
    this.slots.remoteChanged.next(cleared);
  }
