...
> {
  ...
  private readonly _transform = (
    ...
    if (origin instanceof YArray) {
      const attachedOrigin =
        origin.doc || !key
          ? origin
          : this._ySource.get(`prop:${key}`) instanceof YArray
            ? (this._ySource.get(`prop:${key}`) as YArray<unknown>)
            : origin;
      const data = new ReactiveYArray(value as unknown[], attachedOrigin, {
        onChange,
      });
      return data.proxy;
    }
