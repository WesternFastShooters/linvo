...
type TextChangeListener = (event: TextChangeEvent) => void;
 ...
 * text.join(new Text(' linvo'));
 * text.split(7, 1);
 * ```
 *
 * Text delta follows the shared rich-text delta format used by the store.
 *
 * @category Reactive
 */
export class Text<
...
> {
  ...
  private readonly _textChangeListeners = new Set<TextChangeListener>();
  ...
  constructor(
    input?:
      ...
      | { length: number; toDelta(): DeltaOperation[] }
  ) {
    let length = 0;
    if (input instanceof Text) {
      this._yText = input._yText;
      length = input.length;
      ...
      this._yText = yText;
    } else if (input && typeof input === 'object' && 'toDelta' in input) {
    ...
    this._yText.observe(event => {
      ...
      this._onChange?.(this, isLocal);
      this._textChangeListeners.forEach(listener => listener({ isLocal }));
    });
  }

  transact(callback: () => void, withoutTransact = false) {
    ...
    doc.transact(() => {
      callback();
    }, withoutTransact ? null : doc.clientID);
  }
   ...
   * ```ts
   * const text = new Text('Hello, world!');
   * text.applyDelta([{insert: ' linvo', attributes: { bold: true }}]);
   * ```
   */
  ...
  createCursor(index: number) {
    return Y.createRelativePositionFromTypeIndex(this._yText, index);
  }

  observe(listener: TextChangeListener) {
    this._textChangeListeners.add(listener);
  }
  ...
  resolveCursor(cursor: unknown) {
    ...
    return (
      Y.createAbsolutePositionFromRelativePosition(
        cursor as Y.RelativePosition,
        doc
      )?.index ?? null
    );
  }

  unobserve(listener: TextChangeListener) {
    this._textChangeListeners.delete(listener);
  }
    ...
    for (let i = 0; i < deltas.length; i++) {
      ...
      if (typeof insert === 'string') {
        if (tmpIndex + insert.length >= index + length) {
          ...
          rightDeltas.push({
            insert: insertRight,
            attributes: deltas[i].attributes as TextAttributes | undefined,
          });
          rightDeltas.push(
            ...(deltas.slice(i + 1) as DeltaInsert<TextAttributes>[])
          );
          break;
        }
