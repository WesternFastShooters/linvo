import { insertWhiteboardWebEmbedByQuickSearchCommand } from '@linvo-primitives/embed/whiteboard-command';
import {
  FrameTool,
  type PresentToolOption,
} from '@linvo-primitives/frame';
import {
  DefaultTool,
...
import {
  ...
  HighlighterTool,
  MagicBrushTool,
} from '@linvo-primitives/brush';
import {
  ConnectorPathGenerator,
  ConnectorTool,
  mountConnectorLabelEditor,
} from '@linvo-primitives/connector';
import {
  createGroupFromSelectedCommand,
  ungroupCommand,
} from '@linvo-primitives/group';
import {
  getNearestTranslation,
  isElementOutsideViewport,
  isSingleMindMapNode,
} from '@linvo-primitives/mindmap';
import { PanTool } from '@linvo-primitives/pointer';
import { mountShapeTextEditor, ShapeTool } from '@linvo-primitives/shape';
import { createSurfaceText, TextTool } from '@linvo-primitives/text';
import {
...
} from '@linvo-ui/edgeless-selected-rect';
import { IS_MAC } from '@linvo-core/global/env';
import { Bound, getCommonBound } from '@linvo-core/global/gfx';
import { SurfaceSelection, TextSelection } from '@linvo-core/std';
import {
  type BaseTool,
...
import { CommonKeyboardManager } from '../keyboard/common-keyboard-manager';
import type { EdgelessRootBlockComponent } from './edgeless-root-block';
import { ShapePreviewOverlay } from './shape-preview-overlay';
import { deleteElements } from './utils/crud';
import { isCanvasElement } from './utils/query';

export class EdgelessPageKeyboardManager extends CommonKeyboardManager {
    ...
    this.rootComponent.bindHotKey(
      {
        ...
        '@': () => {
          ...
          if (std.selection.find(TextSelection) || Boolean(std.selection.find(SurfaceSelection)?.editing)) {
            return;
          }
          const [_, { insertedLinkType }] = std.command.exec(
            insertWhiteboardWebEmbedByQuickSearchCommand
          );
