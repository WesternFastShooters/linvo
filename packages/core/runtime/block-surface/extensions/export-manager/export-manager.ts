import { ImageBlockModel, type RootBlockModel } from '@linvo-core/content';
import { FetchUtils } from '@linvo-core/shared/adapters';
import {
  CANVAS_EXPORT_IGNORE_TAGS,
  DEFAULT_IMAGE_PROXY_ENDPOINT,
} from '@linvo-core/shared/consts';
import type { Viewport } from '@linvo-core/shared/types';
import {
  isInsidePageEditor,
  matchModels,
...
import { getBgGridGap } from '../../utils/get-bg-grip-gap';
...
export class ExportManager {
    ...
    const html2canvasOption = {
      ignoreElements: function (element: Element) {
        if (
          ...
          // the close and expand buttons in linvo-doc-meta-data is not needed to be showed
          return true;
        } else {
          return false;
        }
  ...
  private async _html2canvas(
    ...
    const html2canvasOption = {
      ...
      onclone: async (documentClone: Document, element: HTMLElement) => {
        ...
        const layer = element.classList.contains('.linvo-edgeless-layer')
          ? element
          : null;
  ...
  async edgelessToCanvas(
    ...
    const container = rootComponent.querySelector(
      '.linvo-block-children-container'
    );
