import type { Color } from '@linvo-core/content';
import {
  FeatureFlagService,
  ThemeProvider,
} from '@linvo-core/shared/services';
import { Bound } from '@linvo-core/global/gfx';
import type { EditorHost, SurfaceSelection } from '@linvo-core/std';
import { BlockComponent } from '@linvo-core/std';
import { GfxControllerIdentifier, type Viewport } from '@linvo-core/std/gfx';
import { RANGE_SYNC_EXCLUDE_ATTR } from '@linvo-core/std/inline';
import { css, html } from 'lit';
import { query } from 'lit/decorators.js';
import type { Subject } from 'rxjs';

import { ConnectorElementModel } from './element-model';
import { CanvasRenderer } from './renderer/canvas-renderer';
import { DomRenderer } from './renderer/dom-renderer';
import { OverlayIdentifier } from './renderer/overlay';
...
export class SurfaceBlockComponent extends BlockComponent<SurfaceBlockModel> {
  ...
  static override styles = css`
    .linvo-edgeless-surface-block-container {
      width: 100%;
      height: 100%;
      position: relative;
    }

    .linvo-edgeless-surface-block-container canvas {
      left: 0;
      top: 0;
      width: 100%;
      height: 100%;
    ...
    .linvo-block-children-container.edgeless {
      padding-left: 0;
      position: relative;
      overflow: hidden;
      height: 100%;
    ...
    .linvo-edgeless-block-child {
      position: absolute;
      transform-origin: center;
      box-sizing: border-box;
      border: 2px solid var(--linvo-white-10);
      border-radius: 8px;
  ...
  override render() {
    return html`
      <div class="linvo-edgeless-surface-block-container">
        <!-- attach canvas later in renderer -->
      </div>
    `;
  }

  @query('.linvo-edgeless-surface-block-container')
  private accessor _surfaceContainer!: HTMLElement;
}

declare global {
  interface HTMLElementTagNameMap {
    'linvo-surface': SurfaceBlockComponent;
  }
}
