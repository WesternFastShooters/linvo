import type { Bound, SerializedXYWH, XYWH } from '@linvo-core/global/gfx';
import { deserializeXYWH } from '@linvo-core/global/gfx';
import { assertType, noop } from '@linvo-core/global/utils';
import type {
  BaseElementProps,
  GfxModel,
...
import { ConnectorMode } from '../../definitions/connector';
import { LayoutType, MindmapStyle } from '../../definitions/mindmap';
import { LocalConnectorElementModel } from '../connector/local-connector-element-model';
import type { MindmapStyleGetter } from './mindmap-style';
import { mindmapStyleGetters } from './mindmap-style';
import { findInfiniteLoop } from './mindmap-utils';
...
function observeChildren(
  ...
  transaction: { local?: boolean } | null
) {
...
export class MindmapElementModel extends GfxGroupLikeElementModel<MindmapElementProps> {
  ...
  static propsToY(props: Record<string, unknown>) {
    if (
      ...
      !(props.children instanceof ElementDataMap)
    ) {
      const children: ElementDataMap = new ElementDataMap();
  ...
  addNode(
    ...
    if (typeof props.text === 'string' || !props.text) {
      props['text'] = new PlainText((props['text'] as string) ?? 'New node');
    }
  ...
  buildTree() {
    ...
    nodesMap.forEach((value, id) => {
      const val = value as NodeDetail;
      ...
      if (!val.parent) {
        ...
        if (!mindmapNodeMap.has(val.parent)) {
          mindmapNodeMap.set(val.parent, {
            id: val.parent,
            detail: nodesMap.get(val.parent)! as NodeDetail,
            parent: null,
            children: [],
  ...
  getParentNode(id: string) {
    const node = this.children.get(id) as NodeDetail | undefined;
   ...
   * @param node
   * @returns a function that writes back the stashed xywh into the element model
   */
  ...
  @convert((initialValue, instance) => {
    if (!(initialValue instanceof ElementDataMap)) {
      nodeSchema.parse(initialValue);

      assertType<NodeType>(initialValue);

      const map: ElementDataMap = new ElementDataMap();
