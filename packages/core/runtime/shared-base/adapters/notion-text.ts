import { DefaultTheme } from '@linvo-core/content';
...
export class NotionTextAdapter extends BaseAdapter<NotionText> {
  ...
  override fromSliceSnapshot():
    ...
    return {
      file: JSON.stringify({
        blockType: 'text',
        editing: [
          ['Notion Text is not supported to be exported from Linvo', []],
        ],
      }),
      assetsIds: [],
    };
  ...
  override toSliceSnapshot(
    ...
    for (const editing of notionText.editing) {
      const delta: DeltaInsert<LinvoTextAttributes> = {
        insert: editing[0],
      };
    ...
    return {
      type: 'slice',
      content,
      docSourceId: payload.docSourceId,
