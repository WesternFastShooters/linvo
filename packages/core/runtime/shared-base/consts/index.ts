import {
  type EmbedCardStyle,
  EmbedFigmaModel,
  EmbedGithubModel,
  EmbedHtmlModel,
  EmbedLoomModel,
  EmbedSyncedDocModel,
  EmbedYoutubeModel,
  SYNCED_DEFAULT_WIDTH,
} from '@linvo-core/content';

export {
  BLOCK_CHILDREN_CONTAINER_PADDING_LEFT,
  EDGELESS_BLOCK_CHILD_BORDER_WIDTH,
  EDGELESS_BLOCK_CHILD_PADDING,
} from './layout';

// The height of the header, which is used to calculate the scroll offset
// Reserve header space so option elements are not covered while scrolling.
export const PAGE_HEADER_HEIGHT = 53;

export const EMBED_CARD_MIN_WIDTH = 450;

export const EMBED_CARD_WIDTH: Record<EmbedCardStyle, number> = {
  horizontal: 752,
  horizontalThin: 752,
  list: 752,
  vertical: 364,
  cube: 170,
  cubeThick: 170,
  video: 752,
  figma: 752,
  html: 752,
  syncedDoc: SYNCED_DEFAULT_WIDTH,
  pdf: 537 + 24 + 2,
  citation: 752,
};

export const EMBED_CARD_HEIGHT: Record<EmbedCardStyle, number> = {
  horizontal: 116,
  horizontalThin: 80,
  list: 46,
  vertical: 390,
  cube: 114,
  cubeThick: 132,
  video: 544,
  figma: 544,
  html: 544,
  syncedDoc: 455,
  pdf: 759 + 46 + 24 + 2,
  citation: 52,
};

export const EMBED_BLOCK_FLAVOUR_LIST = [
  'linvo:embed-github',
  'linvo:embed-youtube',
  'linvo:embed-figma',
  'linvo:embed-synced-doc',
  'linvo:embed-html',
  'linvo:embed-loom',
] as const;

export const EMBED_BLOCK_MODEL_LIST = [
  EmbedGithubModel,
  EmbedYoutubeModel,
  EmbedFigmaModel,
  EmbedSyncedDocModel,
  EmbedHtmlModel,
  EmbedLoomModel,
] as const;

export const DEFAULT_IMAGE_PROXY_ENDPOINT = (() => {
  if (typeof globalThis.location !== 'undefined') {
    const { hostname } = globalThis.location;
    if (
      hostname === 'localhost' ||
      hostname === '127.0.0.1' ||
      hostname === '0.0.0.0'
    ) {
      return '/api/worker/image-proxy';
    }
  }

  return 'https://linvo-worker.toeverything.workers.dev/api/worker/image-proxy';
})();

// https://github.com/toeverything/linvo-workers/tree/main/packages/link-preview
export const DEFAULT_LINK_PREVIEW_ENDPOINT = (() => {
  if (typeof globalThis.location !== 'undefined') {
    const { hostname } = globalThis.location;
    if (
      hostname === 'localhost' ||
      hostname === '127.0.0.1' ||
      hostname === '0.0.0.0'
    ) {
      return '/api/worker/link-preview';
    }
  }

  return 'https://linvo-worker.toeverything.workers.dev/api/worker/link-preview';
})();

// This constant is used to ignore tags when exporting using html2canvas
export const CANVAS_EXPORT_IGNORE_TAGS = [
  'EDGELESS-TOOLBAR-WIDGET',
  'LINVO-DRAG-HANDLE-WIDGET',
  'LINVO-TOOLBAR-WIDGET',
  'LINVO-BLOCK-SELECTION',
];

export * from './bracket-pairs';
export * from './layout';
export * from './note';
export * from './text';
