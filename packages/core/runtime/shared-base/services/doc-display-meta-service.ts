import type { AliasInfo, ReferenceParams } from '@linvo-core/content';
import { type Container, createIdentifier } from '@linvo-core/composition/di';
import {
  type DisposableMember,
  disposeMember,
} from '@linvo-core/global/disposable';
import {
  AliasIcon,
  BlockLinkIcon,
  DeleteIcon,
...
import { LifeCycleWatcher, StdIdentifier } from '@linvo-core/std';
...
import { referenceToNode } from '../utils/reference';
  ...
  implements DocDisplayMetaExtension
{
  ...
  icon(
    ...
    const doc =
      this.std.docSource.doc.id === pageId ? this.std.docSource.doc : null;
  ...
  title(
    ...
    if (!title$) {
      ...
      const disposable = this.std.docSource.slots.docListUpdated.subscribe(
        () => {
