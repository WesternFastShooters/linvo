import { type LinkPreviewData } from '@linvo-core/content';
...
export const getFallbackLinkPreviewData = (
  url: string
  ...
  try {
    ...
    return {
      title: hostname || parsedUrl.host || url,
    };
  } catch {
    return {
      title: url,
...
export const LinkPreviewServiceIdentifier =
  createIdentifier<LinkPreviewProvider>('LinvoLinkPreviewService');
  ...
  implements LinkPreviewProvider
{
  ...
  private readonly _fetchStandardPreview = async (
    ...
    const fallbackData = getFallbackLinkPreviewData(url);
    ...
    const data: LinkPreviewResponseData | null = await response
      .json()
      .catch(() => null);
    if (!data) {
      return fallbackData;
    ...
    return {
      title: data.title ?? fallbackData.title ?? null,
