import { NoteBlockModel, RootBlockModel } from '@linvo-core/content';
import { clamp, type Point, type Rect } from '@linvo-core/global/gfx';
import { BLOCK_ID_ATTR, type BlockComponent } from '@linvo-core/std';
import { SurfaceBlockModel } from '@linvo-core/std/gfx';
import type { BlockModel } from '@linvo-core/store';

import { BLOCK_CHILDREN_CONTAINER_PADDING_LEFT } from '../../consts';
import { matchModels } from '../document/checker';

const ATTR_SELECTOR = `[${BLOCK_ID_ATTR}]`;

// margin-top: calc(var(--linvo-paragraph-space) + 24px);
...
function isImage({ tagName }: Element) {
  return tagName === 'LINVO-IMAGE';
}

function isDatabase({ tagName }: Element) {
  return tagName === 'LINVO-DATABASE';
}
...
export function getClosestBlockComponentByPoint(
  ...
  if (element) {
    ...
    if (isDatabase(element)) {
      ...
      childBounds = element
        .querySelector('.linvo-block-children-container')
 ...
 * https://github.com/toeverything/linvo/issues/902
 * https://github.com/toeverything/linvo/pull/1121
 */
 ...
 * In Chrome/Safari, `document.elementsFromPoint` does not include `linvo-image`.
 */
...
function getDatabaseBlockRowsElement(element: Element) {
  return element.querySelector('.linvo-database-block-rows');
}
