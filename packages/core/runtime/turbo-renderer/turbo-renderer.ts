 ...
 *
 * 1.  **In the block's package (e.g., `packages/blocks/my-block`):**
 *   a.  Add `@linvo-core/turbo-renderer` as a dependency in `package.json` and create a `src/turbo` directory.
 *   b.  Implement the Layout Handler (e.g., `MyBlockLayoutHandlerExtension`) and Painter Worker (e.g., `MyBlockLayoutPainterExtension`). Refer to `ParagraphLayoutHandlerExtension` and `ParagraphLayoutPainterExtension` in `packages/blocks/block-paragraph` for implementation examples.
 ...
 *   e.  Add a TypeScript project reference to `packages/gfx/turbo-renderer` in `tsconfig.json`.
 *
 * 2.  **In the application integration point (e.g., the app extension bootstrap and the test renderer entry):**
 ...
 *
 * 3.  **Run `yarn linvo init`** from the repository root to update generated configuration files and the lockfile (`yarn.lock`).
 *
 * **Note:** Always ensure the directory structure and export patterns match the `paragraph` block (`packages/blocks/block-paragraph`) for consistency.
 */
