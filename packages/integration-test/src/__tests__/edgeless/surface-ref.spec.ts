...
import type { DocSnapshot } from '@linvo/store';
...
describe('basic', () => {
  ...
  test('surface-ref should be rendered in page mode', async () => {
    ...
    expect(
      document.querySelector(
        `linvo-surface-ref[data-block-id="${surfaceRefId}"]`
      )
    ).instanceOf(Element);
  });
  ...
  test('content in frame should be rendered in the correct order', async () => {
    ...
    const refBlocks = Array.from(
      surfaceRef.querySelectorAll('linvo-edgeless-note')
    ) as HTMLElement[];
...
import type { SurfaceRefBlockComponent } from '@linvo/linvo/blocks/surface-ref';
...
describe('clipboard', () => {
  test('import surface-ref snapshot should render content correctly', async () => {
    ...
    const surfaceRefs = newDoc!.getBlocksByFlavour('linvo:surface-ref');
