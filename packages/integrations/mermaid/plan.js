...
function resolveShapeAlias(value) {
    return shapeAliasMap[value.toLowerCase().replace(/[^a-z]/g, '')] ?? null;
}
function parseLegacyShape(shape) {
    const patterns = [
        [/^\(\((.+)\)\)$/s, 'ellipse'],
        [/^\(\(\((.+)\)\)\)$/s, 'doubleCircle'],
        [/^\(\[(.+)\]\)$/s, 'stadium'],
...
function parseNodeToken(token) {
    ...
    if (shape.startsWith('@{')) {
        ...
        return {
            id,
            label: stripQuotes((labelMatch?.[2] ?? labelMatch?.[3] ?? legacy?.label ?? id)) || id,
            shapeType,
        };
    }
...
function parseFlowchart(code) {
    ...
    if (!header)
        return null;
    const direction = header[2].toUpperCase();
    const nodes = new Map();
    const edges = [];
...
export function buildMermaidInsertPlan(renderResult) {
