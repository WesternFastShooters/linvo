...
const TEXT_FONT = '"Comic Sans MS", "Marker Felt", "Segoe Print", ui-rounded, sans-serif';
...
function appendNode(svg, rough, node) {
    ...
    switch (node.shapeType) {
        ...
        default:
            ...
            shape = rough.polygon(shapeMethods[node.shapeType].points({
                x,
                y,
                w: node.width,
                h: node.height,
    ...
    lines.forEach((line, index) => {
        ...
        tspan.setAttribute('dy', index === 0 ? `${-(lines.length - 1) * 0.55}em` : '1.1em');
        tspan.textContent = line;
        text.append(tspan);
    });
    svg.append(text);
}
function appendArrow(svg, edge, nodeMap) {
    const source = nodeMap.get(edge.sourceId);
    const target = nodeMap.get(edge.targetId);
    if (!source || !target) {
        return;
    }
    const [startX, startY] = getNodeAnchor(source, edge.direction === 'left'
        ? 'left'
        : edge.direction === 'right'
            ...
            : edge.direction === 'top'
                ? 'top'
                : 'bottom');
    const opposite = edge.direction === 'left'
        ? 'right'
        : edge.direction === 'right'
            ? 'left'
            : edge.direction === 'top'
...
export function buildMermaidPreviewModel(plan, fallbackSvg) {
    if (plan.kind !== 'native') {
        return {
            mode: 'image',
            svg: fallbackSvg,
