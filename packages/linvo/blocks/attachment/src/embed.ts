...
import { FileSizeLimitProvider } from '@linvo/linvo-shared/services';
import {
  readImageSize,
  transformModel,
  withTempBlobData,
...
import { type BlockStdScope, StdIdentifier } from '@linvo/std';
...
export const AttachmentEmbedConfigIdentifier =
  createIdentifier<AttachmentEmbedConfig>(
    'LinvoAttachmentEmbedConfigIdentifier'
  );
...
const embedConfig: AttachmentEmbedConfig[] = [
  {
    ...
    check: model =>
      model.store.schema.flavourSchemaMap.has('linvo:image') &&
