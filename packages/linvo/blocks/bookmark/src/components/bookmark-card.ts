import { getEmbedCardIcons } from '@linvo/linvo-block-embed';
import { LoadingIcon, WebIcon16 } from '@linvo/linvo-components/icons';
...
export class BookmarkCard extends SignalWatcher(
  ...
  override render() {
    ...
    return html`
      <div
        class="linvo-bookmark-card ${cardClassMap}"
        @click=${this.bookmark.handleClick}
        @dblclick=${this.bookmark.handleDoubleClick}
      >
        <div class="linvo-bookmark-content">
          <div class="linvo-bookmark-content-title">
            <div class="linvo-bookmark-content-title-icon">${titleIcon}</div>
            <div class="linvo-bookmark-content-title-text">${titleText}</div>
          </div>
          <div class="linvo-bookmark-content-description">
            ${descriptionText}
          </div>
          <div class="linvo-bookmark-content-url-wrapper">
            <div
              class="linvo-bookmark-content-url"
              @click=${this.bookmark.open}
            >
              <span>${getHostName(url)}</span>
              <div class="linvo-bookmark-content-url-icon">
                ${OpenInNewIcon({ width: '12', height: '12' })}
              </div>
            </div>
          </div>
        </div>
        <div class="linvo-bookmark-banner">${bannerImage}</div>
      </div>
    `;
  }
