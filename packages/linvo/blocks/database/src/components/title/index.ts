...
import type { DataViewUILogicBase } from '@linvo/data-view';
...
export class DatabaseTitle extends SignalWatcher(
  ...
  static override styles = css`
    ...
    .linvo-database-title .text {
      user-select: none;
      opacity: 0;
      white-space: pre-wrap;
    }

    .linvo-database-title[data-title-focus='false'] textarea {
      opacity: 0;
    }

    .linvo-database-title[data-title-focus='false'] .text {
    ...
    .linvo-database-title [data-title-empty='true']::before {
      content: 'Untitled';
      position: absolute;
      pointer-events: none;
      color: var(--linvo-text-primary-color);
    ...
    .linvo-database-title [data-title-focus='true']::before {
      color: var(--linvo-placeholder-color);
    }

    .linvo-database-title.comment-highlighted {
  ...
  get database() {
    return this.closest<DatabaseBlockComponent>('linvo-database');
  }
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-database-title': DatabaseTitle;
  }
}
