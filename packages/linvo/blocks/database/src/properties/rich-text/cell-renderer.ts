...
import type { BlockSnapshot, DeltaInsert } from '@linvo/store';
...
function toggleStyle(
  inlineEditor: LinvoInlineEditor | null,
  attrs: LinvoTextAttributes
): void {
  ...
  let oldAttributes: LinvoTextAttributes = {};
...
export class RichTextCell extends BaseCellRenderer<Text, string> {
  ...
  get topContenteditableElement() {
    const databaseBlock =
      this.closest<DatabaseBlockComponent>('linvo-database');
  ...
  insertDelta = (delta: DeltaInsert<LinvoTextAttributes>) => {
    const inlineEditor = this.inlineEditor$.value;
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-database-rich-text-cell': RichTextCell;
  }
}
