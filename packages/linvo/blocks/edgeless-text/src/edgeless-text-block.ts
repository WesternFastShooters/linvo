...
import { formatBlockCommand } from '@linvo/linvo-inline-preset';
import {
  DefaultTheme,
...
import { Bound, clamp } from '@linvo/global/gfx';
...
export class EdgelessTextBlockComponent extends GfxBlockComponent<EdgelessTextBlockModel> {
  static override styles = css`
    ...
    .edgeless-text-block-container linvo-paragraph,
    linvo-list {
  ...
  override renderGfxBlock() {
    ...
    const containerStyle: StyleInfo = {
      ...
      border: `1px solid ${editing ? 'var(--linvo—primary—color, #1e96eb)' : 'transparent'}`,
      borderRadius: '4px',
  ...
  override renderPageContent() {
    ...
    const style = styleMap({
      ...
      '--linvo-list-margin': '0',
      '--linvo-paragraph-margin': '0',
    });

    return html`
      <div style=${style} class="linvo-block-children-container">
  ...
  tryFocusEnd() {
    const paragraphOrLists = Array.from(
      this.querySelectorAll<BlockComponent>('linvo-paragraph, linvo-list')
    );
  ...
  @query('.linvo-block-children-container')
  accessor childrenContainer!: HTMLDivElement;
}

declare global {
  interface HTMLElementTagNameMap {
    'linvo-edgeless-text': EdgelessTextBlockComponent;
  }
}
