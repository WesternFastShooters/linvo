import {
  EmbedBlockComponent,
  isEmptyDoc,
} from '@linvo/linvo-block-embed';
import { Peekable } from '@linvo/linvo-components/peek';
import { ViewExtensionManagerIdentifier } from '@linvo/linvo-ext-loader';
import {
  type DocLinkClickedEvent,
  RefNodeSlotsProvider,
} from '@linvo/linvo-inline-reference';
import {
  type AliasInfo,
  type DocMode,
...
import { cloneReferenceInfo } from '@linvo/linvo-shared/utils';
import { Bound, getCommonBound } from '@linvo/global/gfx';
import {
  BlockSelection,
  BlockStdScope,
  type EditorHost,
...
import { GfxControllerIdentifier, GfxExtension } from '@linvo/std/gfx';
import { type GetStoreOptions, type Query, Text } from '@linvo/store';
...
export class EmbedSyncedDocBlockComponent extends EmbedBlockComponent<EmbedSyncedDocModel> {
    ...
    class EmbedSyncedDocWatcher extends LifeCycleWatcher {
      ...
      override mounted(): void {
        ...
        view.viewUpdated.subscribe(payload => {
          if (
            payload.type !== 'block' ||
            payload.view.model.flavour !== 'linvo:embed-synced-doc'
          ) {
            return;
          }
  ...
  protected _renderSyncedView = () => {
    ...
    return this.renderEmbed(
      () => html`
        <div
          class=${classMap({
            'linvo-embed-synced-doc-container': true,
            [editorMode]: true,
            [theme]: true,
            surface: false,
        ...
        >
          ...
          <div
            class=${classMap({
              'linvo-embed-synced-doc-header-wrapper': true,
              selected: this.selected$.value,
            })}
          >
            <div class="linvo-embed-synced-doc-header">
              <span class="linvo-embed-synced-doc-icon"
                >${this.icon$.value}</span
              >
              <span class="linvo-embed-synced-doc-title">${this.title$}</span>
            </div>
          </div>
        </div>
      `
  ...
  @query(
    ':scope > .linvo-block-component > .embed-block-container > linvo-embed-synced-doc-card'
  )
  accessor syncedDocCard: EmbedSyncedDocCard | null = null;

  @query(
    ':scope > .linvo-block-component > .embed-block-container > .linvo-embed-synced-doc-container > .linvo-embed-synced-doc-editor > div > editor-host'
  )
