...
import type { IPoint, IVec } from '@linvo/global/gfx';
import { Bound, Vec } from '@linvo/global/gfx';
import type { PointerEventState } from '@linvo/std';
import { BaseTool, getTopElements } from '@linvo/std/gfx';
  ...
  override dragMove(e: PointerEventState): void {
    ...
    if (!this._frame) {
      const frames = this.gfx.layer.blocks.filter(
        block => block.flavour === 'linvo:frame'
      ) as FrameBlockModel[];
