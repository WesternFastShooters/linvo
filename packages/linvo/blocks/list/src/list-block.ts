import '@linvo/linvo-shared/commands';

import { CaptionedBlockComponent } from '@linvo/linvo-components/caption';
import { playCheckAnimation } from '@linvo/linvo-components/icons';
import { TOGGLE_BUTTON_PARENT_CLASS } from '@linvo/linvo-components/toggle-button';
import { DefaultInlineManagerExtension } from '@linvo/linvo-inline-preset';
import type { ListBlockModel } from '@linvo/linvo-model';
import type { RichText } from '@linvo/linvo-rich-text';
import {
...
import { BlockSelection, TextSelection } from '@linvo/std';
import {
  getInlineRangeProvider,
  type InlineRangeProvider,
} from '@linvo/std/inline';
import type { BaseSelection } from '@linvo/store';
...
export class ListBlockComponent extends CaptionedBlockComponent<ListBlockModel> {
  ...
  private readonly _onClickIcon = (e: MouseEvent) => {
    ...
    if (this.model.props.type === 'toggle') {
      ...
      if (this.model.props.checked) {
        const checkEl = this.querySelector('.linvo-list-block__todo-prefix');
        if (checkEl) {
  ...
  override renderBlock(): TemplateResult<1> {
    ...
    const children = html`<div
      class="linvo-block-children-container"
      style=${styleMap({
    ...
    return html`
      <div class=${'linvo-list-block-container'}>
        <div
          class=${classMap({
            'linvo-list-rich-text-wrapper': true,
            'linvo-list--checked':
  ...
  override accessor blockContainerStyles = {
    margin: 'var(--linvo-list-margin, 10px 0)',
  };
}
