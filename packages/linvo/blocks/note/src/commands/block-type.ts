...
> = (ctx, next) => {
  ...
  const appendDivider: Command<{}, { updatedBlocks: BlockModel[] }> = (
    ...
    const id = doc.addBlock('linvo:divider', {}, parent, index + 1);
    if (!nextSibling) {
      nextSiblingId = doc.addBlock('linvo:paragraph', {}, parent);
    }
  ...
  const [result, resultCtx] = std.command
    ...
    .try(chain => [
      chain.pipe((_, next) => {
        if (['linvo:code', 'linvo:divider'].includes(flavour)) {
          return next();
        }
        return false;
      }),
