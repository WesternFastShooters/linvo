...
import { addImages } from '@linvo/linvo-block-image';
import {
  CanvasRenderer,
  DefaultTool,
...
import { splitIntoLines } from '@linvo/linvo-gfx-text';
import type {
  EmbedCardStyle,
  ShapeElementModel,
...
import { type BlockSnapshot, type SliceSnapshot } from '@linvo/store';
...
export class EdgelessClipboardController extends PageClipboard {
  ...
  private readonly _onPaste = async (_context: UIEventStateContext) => {
    ...
    if (isUrlInClipboard(data)) {
      ...
      // try to interpret url as linvo doc url
  ...
  private async _pasteTextContentAsNote(content: BlockSnapshot[] | string) {
    ...
    if (typeof content === 'string') {
      ...
      for (let index = 0; index < content.length; index++) {
        ...
        if (blockSnapshot.flavour === 'linvo:note') {
