import { appendParagraphCommand } from '@linvo/linvo-block-paragraph';
import {
  CodeBlockModel,
  ListBlockModel,
  NoteBlockModel,
...
import { Point } from '@linvo/global/gfx';
import type { PointerEventState } from '@linvo/std';
import { BlockComponent, BlockSelection, TextSelection } from '@linvo/std';
import type { BlockModel, Text } from '@linvo/store';
...
export class PageRootBlockComponent extends BlockComponent<RootBlockModel> {
  static override styles = css`
    editor-host:has(> linvo-page-root, * > linvo-page-root) {
      display: block;
      height: 100%;
    }
    ...
    .linvo-page-root-block-container {
      ...
      font-weight: 400;
      max-width: var(--linvo-editor-width);
      margin: 0 auto;

      /* Leave a place for drag-handle */
      /* Do not use prettier format this style, or it will be broken */
      /* prettier-ignore */
      padding-left: var(--linvo-editor-side-padding, ${DOC_BLOCK_CHILD_PADDING}px);
      /* prettier-ignore */
      padding-right: var(--linvo-editor-side-padding, ${DOC_BLOCK_CHILD_PADDING}px);
      /* prettier-ignore */
      padding-bottom: var(--linvo-editor-bottom-padding, ${DOC_BOTTOM_PADDING}px);
    }
  ...
  private _createDefaultNoteBlock() {
    ...
    const noteId = store.addBlock('linvo:note', {}, store.root?.id);
  ...
  private _getDefaultNoteBlock() {
    return (
      this.store.root?.children.find(
        child => child.flavour === 'linvo:note'
  ...
  override connectedCallback() {
    ...
    this.bindHotKey({
      ...
      ArrowUp: () => {
        ...
        if (!prevNote || prevNote.flavour !== 'linvo:note') {
        ...
        const notes = this.store.getModelsByFlavour('linvo:note');
  ...
  override renderBlock() {
    ...
    return html`
      <div class="linvo-page-root-block-container">${children} ${widgets}</div>
    `;
  }

  @query('.linvo-page-root-block-container')
