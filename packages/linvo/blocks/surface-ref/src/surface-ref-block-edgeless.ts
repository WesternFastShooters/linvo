import { type SurfaceRefBlockModel } from '@linvo/linvo-model';
import { unsafeCSSVarV2 } from '@linvo/linvo-shared/theme';
import { BlockComponent, BlockSelection } from '@linvo/std';
...
export class EdgelessSurfaceRefBlockComponent extends BlockComponent<SurfaceRefBlockModel> {
  static override styles = css`
    linvo-edgeless-surface-ref {
      position: relative;
      overflow: hidden;
    }

    .linvo-edgeless-surface-ref-container {
      border-radius: 8px;
      border: 1px solid
    ...
    .linvo-edgeless-surface-ref-container.focused {
  ...
  override renderBlock() {
    return html` <div
      class=${classMap({
        'linvo-edgeless-surface-ref-container': true,
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-edgeless-surface-ref': EdgelessSurfaceRefBlockComponent;
  }
}
