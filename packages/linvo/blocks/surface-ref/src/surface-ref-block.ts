import { FrameBlockComponent } from '@linvo/linvo-block-frame';
import {
  EdgelessCRUDIdentifier,
  getSurfaceBlock,
} from '@linvo/linvo-block-surface';
import type { BlockCaptionEditor } from '@linvo/linvo-components/caption';
...
import { requestConnectedFrame } from '@linvo/linvo-shared/utils';
import { DisposableGroup } from '@linvo/global/disposable';
import { LinvoError, ErrorCode } from '@linvo/global/exceptions';
import { Bound, type SerializedXYWH } from '@linvo/global/gfx';
import {
  BlockComponent,
  BlockSelection,
  BlockStdScope,
...
import type { BaseSelection, ExtensionType, Store } from '@linvo/store';
...
export class SurfaceRefBlockComponent extends BlockComponent<SurfaceRefBlockModel> {
  static override styles = css`
    ...
    linvo-surface-ref:not(:hover)
      linvo-surface-ref-toolbar:not([data-open-menu-display='show']) {
      display: none;
    }
    ...
    .linvo-surface-ref.focused {
      border-color: ${unsafeCSSVarV2('edgeless/frame/border/active')};
    }

    .linvo-surface-ref.comment-highlighted {
    ...
    .ref-content {
      ...
      background: radial-gradient(
        var(--linvo-edgeless-grid-color) 1px,
        var(--linvo-background-primary-color) 1px
      );
    }
  ...
  private get _shouldRender() {
    return (
      ...
      !this.parentComponent?.closest('linvo-surface-ref')
    );
  }
  ...
  @query('.linvo-surface-ref')
  accessor hoverableContainer!: HTMLDivElement;

  @query('linvo-surface-ref > block-caption-editor')
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-surface-ref': SurfaceRefBlockComponent;
  }
}
