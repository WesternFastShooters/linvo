...
import type { TableBlockModel } from '@linvo/linvo-model';
...
import { VirtualPaddingController } from '@linvo/linvo-shared/utils';
...
export const TableBlockComponentName = 'linvo-table';
        ...
        >
          <table class=${tableWrapper} ${ref(this.table$)}>
            ...
            ${IS_MOBILE || this.dataManager.readonly$.value
              ? nothing
              : html`<linvo-table-add-button
                  style="display: contents;"
                  .dataManager=${this.dataManager}
                ></linvo-table-add-button>`}
            ${html`<linvo-table-selection-layer
              style="display: contents;"
              .selectionController=${this.selectionController}
              .getRowRect=${this.getRowRect}
              .getColumnRect=${this.getColumnRect}
              .getAreaRect=${this.getAreaRect}
            ></linvo-table-selection-layer>`}
          </table>
        </div>
      </div>
    `;
