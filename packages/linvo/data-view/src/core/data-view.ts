...
import type { DisposableMember } from '@linvo/global/disposable';
...
export class DataViewRootUILogic {
  ...
  private createDataViewUILogic(viewId: string): DataViewUILogicBase {
    ...
    if (!view) {
      throw new LinvoError(
        LinvoError.ErrorCode.DatabaseBlockError,
  ...
  render() {
    return html` <linvo-data-view-renderer
      ${ref(this._uiRef)}
      .logic="${this}"
    ></linvo-data-view-renderer>`;
  }
}
...
export class DataViewRootUI extends SignalWatcher(
  ...
  static override styles = css`
    ${unsafeCSS(dataViewCommonStyle('linvo-data-view-renderer'))}
    linvo-data-view-renderer {
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-data-view-renderer': DataViewRootUI;
  }
}
