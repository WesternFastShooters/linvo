...
const styles = css`
  ${unsafeCSS(dataViewCommonStyle('linvo-data-view-record-detail'))}
  linvo-data-view-record-detail {
    position: relative;
    display: flex;
    flex: 1;
...
export class RecordDetail extends SignalWatcher(
  ...
  override connectedCallback() {
    ...
    this.dataset.widgetId = 'linvo-detail-widget';
  }
  ...
  override render() {
    ...
    return html`
      ...
      <div style="flex:1;overflow-y: auto;overflow-x: hidden">
        <div
          style="width: 100%;max-width: var(--linvo-editor-width);display: flex;flex-direction: column;margin: 0 auto;box-sizing: border-box;"
        >
          ...
          ${repeat(
            ...
            property => {
              return keyed(
                this.rowId,
                html` <linvo-data-view-record-field
                  .view="${this.view}"
                  .column="${property}"
                  .rowId="${this.rowId}"
                  data-column-id="${property.id}"
                ></linvo-data-view-record-field>`
              );
            }
          )}
          ${!this.readonly
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-data-view-record-detail': RecordDetail;
  }
}
export const createRecordDetail = (ops: {
  ...
  return html` <linvo-data-view-record-detail
    .view=${ops.view}
    .rowId=${ops.rowId}
