...
export class RecordField extends SignalWatcher(
  ...
  static override styles = css`
    ...
    linvo-data-view-record-field .icon {
      display: flex;
      align-items: center;
      width: 16px;
      height: 16px;
    ...
    linvo-data-view-record-field .icon svg {
      width: 16px;
      height: 16px;
    ...
    .field-content .linvo-database-number {
      text-align: left;
  ...
  changeEditing = (editing: boolean) => {
    const selection = this.closest('linvo-data-view-record-detail')?.selection;
    if (selection) {
      selection.selection = {
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-data-view-record-field': RecordField;
  }
}
