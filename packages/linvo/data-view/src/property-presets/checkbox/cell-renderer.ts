...
const playCheckAnimation = async (
  ...
  sparkingEl.classList.add('linvo-check-animation');
  if (size < 20) {
...
export class CheckboxCell extends BaseCellRenderer<boolean> {
  static override styles = css`
    linvo-database-checkbox-cell {
      display: block;
      width: 100%;
      cursor: pointer;
    }

    .linvo-database-checkbox-container {
      height: 100%;
    }

    .linvo-database-checkbox {
      display: flex;
      align-items: center;
  ...
  override render() {
    ...
    const icon = checked
      ...
      : CheckBoxUnIcon();
    return html` <div class="linvo-database-checkbox-container">
      <div
        class="linvo-database-checkbox checkbox ${checked ? 'checked' : ''}"
      >
        ${icon}
      </div>
    </div>`;
