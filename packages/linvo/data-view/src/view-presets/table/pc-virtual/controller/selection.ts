import { getRangeByPositions } from '@linvo/linvo-shared/utils';
...
export class TableSelectionController implements ReactiveController {
  ...
  private resolveDragStartTarget(
    ...
    if (fillValues) {
      ...
      cell = target.closest('linvo-database-virtual-cell-container');
    }
  ...
  getCellElement(cell: TableGridCell): DatabaseCellContainer | undefined {
    return (
      cell.element.querySelector('linvo-database-virtual-cell-container') ??
      undefined
    );
  }
...
export class SelectionElement extends SignalWatcher(
  ...
  static override styles = css`
    ...
    .database-selection {
      ...
      background-color: var(--linvo-primary-color-04);
      pointer-events: none;
      display: none;
    }
