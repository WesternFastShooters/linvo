...
export const getTableGroupRect = (ele: HTMLElement) => {
  const group = ele.closest('linvo-data-view-virtual-table-group');
  if (!group) {
    return;
  }
  ...
  const top =
    group
      .querySelector('.linvo-database-column-header')
      ?.getBoundingClientRect().top ?? groupRect.top;
  const bottom =
    group.querySelector('.linvo-database-block-rows')?.getBoundingClientRect()
