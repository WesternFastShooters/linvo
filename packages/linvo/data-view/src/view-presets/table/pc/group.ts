...
const styles = css`
  linvo-data-view-table-group:hover .group-header-op {
    visibility: visible;
    opacity: 1;
  }
...
export class TableGroup extends SignalWatcher(
  ...
  private renderRows(rows: Row[]) {
    return html`
      ...
      ></linvo-database-column-header>
      <div class="linvo-database-block-rows">
        ${repeat(
          rows,
          row => row.rowId,
          (row, idx) => {
        ...
        : html` <div
          ...
          >
            <div
              ...
              data-test-id="linvo-database-add-row-button"
              role="button"
            >
              ${PlusIcon()}<span style="font-size: 12px">New Record</span>
            </div>
          </div>`}
      <linvo-database-column-stats
        .tableViewLogic="${this.tableViewLogic}"
        .group="${this.group}"
      >
      </linvo-database-column-stats>
  ...
  @query('.linvo-database-block-rows')
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-data-view-table-group': TableGroup;
  }
}
