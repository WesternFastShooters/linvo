...
export class DatabaseColumnHeader extends SignalWatcher(
  ...
  editLastColumnTitle = () => {
    const columns = this.querySelectorAll('linvo-database-header-column');
  ...
  override connectedCallback() {
    ...
    const scrollContainer = getScrollContainer(
      this.closest('linvo-data-view-renderer')!
    );
    const group = this.closest('linvo-data-view-table-group');
    if (group) {
  ...
  override render() {
    return html`
      ...
      <div class="linvo-database-column-header database-row">
        ${this.readonly
          ? nothing
          ...
          (column, index) => {
            ...
            return html`
              <linvo-database-header-column
                ...
                class="linvo-database-column database-cell"
                .column="${column}"
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-database-column-header': DatabaseColumnHeader;
  }
}
