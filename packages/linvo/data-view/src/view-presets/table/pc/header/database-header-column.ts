...
export class DatabaseHeaderColumn extends SignalWatcher(
  ...
  static override styles = css`
    linvo-database-header-column {
      display: flex;
    }

    .linvo-database-header-column-grabbing * {
      cursor: grabbing;
    }
  `;
                    ...
                    items: [
                      ...
                      ...numberFormats.map(format => {
                        ...
                        return menu.action({
                          ...
                          prefix: html`<span
                            style="font-size: var(--linvo-font-base); scale: 1.2;"
                            >${format.symbol}</span
                          >`,
                          name: format.label,
                          select: () => {
  ...
  override render() {
    ...
    const classes = classMap({
      'linvo-database-column-move': true,
      [this.grabStatus]: true,
    });
    return html`
      <div
        style=${style}
        class="linvo-database-column-content"
      ...
      >
        ...
        <div class="linvo-database-column-text ${column.type$.value}">
          <div
            class="linvo-database-column-type-icon dv-hover"
            @click="${this._clickTypeIcon}"
          >
            <uni-lit .uni="${column.icon}"></uni-lit>
          </div>
          <div class="linvo-database-column-text-content">
            <div class="linvo-database-column-text-input">
              ${column.name$.value}
            </div>
          </div>
        </div>
...
function numberFormatConfig(column: Property): MenuConfig {
  return () =>
    html` <linvo-database-number-format-bar
      .column="${column}"
    ></linvo-database-number-format-bar>`;
}

declare global {
  interface HTMLElementTagNameMap {
    'linvo-database-header-column': DatabaseHeaderColumn;
  }
}
