...
const styles = css`
  .linvo-database-search-container {
    position: relative;
    display: flex;
    align-items: center;
    gap: 8px;
  ...
  .linvo-database-search-input-icon {
    position: absolute;
    left: 0;
    font-size: 20px;
    display: flex;
  ...
  .linvo-database-search-input-icon:hover {
    background: var(--linvo-hover-color);
  }

  .search-container-expand .linvo-database-search-input-icon {
    left: 4px;
    pointer-events: none;
  }

  .linvo-database-search-input {
    flex: 1;
    width: 100%;
  ...
  .linvo-database-search-input::placeholder {
...
> {
  ...
  override render() {
    const searchToolClassMap = classMap({
      'linvo-database-search-container': true,
      'search-container-expand': this.showSearch,
      active: this.showSearch,
    });
    return html`
      <label class="${searchToolClassMap}" @click="${this._clickSearch}">
        <div class="linvo-database-search-input-icon">${SearchIcon()}</div>
        <input
          placeholder="Search..."
          class="linvo-database-search-input"
  ...
  @query('.linvo-database-search-input')
