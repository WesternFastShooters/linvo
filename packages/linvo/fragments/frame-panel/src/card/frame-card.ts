...
import { on, once } from '@linvo/linvo-shared/utils';
...
const styles = css`
  ...
  .frame-card-container.dragging {
    ...
    top: 0;
    left: 0;
    z-index: calc(var(--linvo-z-index-popover, 0) + 3);
  }
...
export const LINVO_FRAME_CARD = 'linvo-frame-card';
...
export class FrameCard extends WithDisposable(ShadowlessElement) {
  ...
  override render() {
    ...
    >
      ${this.status === 'dragging'
        ? nothing
        : html`<linvo-frame-card-title
            .cardIndex=${this.cardIndex}
            .frame=${this.frame}
          ></linvo-frame-card-title>`}
      <div
