...
import type { NoteChildrenFlavour } from '@linvo/linvo-shared/types';
import {
  handleNativeRangeAtPoint,
  hasClassNameInList,
} from '@linvo/linvo-shared/utils';
import { type IPoint, Point, serializeXYWH } from '@linvo/global/gfx';
import type { BlockStdScope, PointerEventState } from '@linvo/std';
import {
  BaseTool,
...
export class NoteTool extends BaseTool<NoteToolOption> {
  static override toolName = 'linvo:note';
  ...
  override activate() {
    const attributes =
      this.std.get(EditPropsStore).lastProps$.value['linvo:note'];
