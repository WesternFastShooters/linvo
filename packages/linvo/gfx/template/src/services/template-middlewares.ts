import { generateElementId, sortIndex } from '@linvo/linvo-block-surface';
...
import type { BlockSnapshot, SnapshotNode } from '@linvo/store';
...
export const replaceIdMiddleware = (job: TemplateJob) => {
  ...
  const regenerateBlockId = (data: SlotBlockPayload['data']) => {
    ...
    if (blockJson.flavour === 'linvo:surface-ref') {
      assertType<
        SnapshotNode<{
          reference: string;
        }>
    ...
    if (blockJson.flavour === 'linvo:surface') {
    ...
    if (blockJson.flavour === 'linvo:frame') {
...
export const createStickerMiddleware = (
  ...
  return (job: TemplateJob) => {
    ...
    const changeInserPosition = (blockJson: BlockSnapshot) => {
      if (blockJson.flavour === 'linvo:image' && blockJson.props.xywh) {
        const bound = Bound.deserialize(blockJson.props['xywh'] as string);
