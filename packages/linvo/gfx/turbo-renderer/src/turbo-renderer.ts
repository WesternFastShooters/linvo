 ...
 *
 * To add Turbo Rendering support for a new block type (e.g., 'linvo:my-block'):
 *
 * 1.  **In the block's package (e.g., `linvo/linvo/blocks/my-block`):**
 *   a.  Add `@linvo/linvo/gfx/turbo-renderer` as a dependency in `package.json` and create a `src/turbo` directory.
 *   b.  Implement the Layout Handler (e.g., `MyBlockLayoutHandlerExtension`) and Painter Worker (e.g., `MyBlockLayoutPainterExtension`). Refer to `ParagraphLayoutHandlerExtension` and `ParagraphLayoutPainterExtension` in `linvo/linvo/blocks/block-paragraph` for implementation examples.
 ...
 *   e.  Add a TypeScript project reference to `linvo/linvo/gfx/turbo-renderer` in `tsconfig.json`.
 *
 * 2.  **In the application integration point (e.g., `packages/frontend/core/src/linvo/extensions` and `linvo/integration-test/src/__tests__/utils/renderer-entry.ts`):**
 ...
 *
 * 3.  **Run `yarn linvo init`** from the workspace root to update generated configuration files (`workspace.gen.ts`) and the lockfile (`yarn.lock`).
 *
 * **Note:** Always ensure the directory structure and export patterns match the `paragraph` block (`linvo/linvo/blocks/block-paragraph`) for consistency.
 */
export class ViewportTurboRendererExtension extends GfxExtension {
  ...
  override mounted() {
    const mountPoint = document.querySelector('.linvo-edgeless-viewport');
    if (mountPoint) {
