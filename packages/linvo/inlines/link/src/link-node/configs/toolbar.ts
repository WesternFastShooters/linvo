...
import { LinvoLink } from '../linvo-link';
  ...
  actions: [
    {
      ...
      content(cx) {
        ...
        if (!(target instanceof LinvoLink)) return null;

        const { link } = target;
        if (!link) return null;

        return html`<linvo-link-preview .url=${link}></linvo-link-preview>`;
      },
    },
    {
      ...
      actions: [
        {
          ...
          run(ctx) {
            ...
            if (!(target instanceof LinvoLink)) return;
    ...
    },
    {
      ...
      actions: [
        ...
        },
        {
          ...
          when(ctx) {
            const target = ctx.message$.peek()?.element;
            if (!(target instanceof LinvoLink)) return false;
      ...
      when(ctx) {
        ...
        const bookmarkSchema = schema.flavourSchemaMap.get('linvo:bookmark');
