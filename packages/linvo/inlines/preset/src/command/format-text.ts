import { clearMarksOnDiscontinuousInput } from '@linvo/linvo-rich-text';
import { getSelectedBlocksCommand } from '@linvo/linvo-shared/commands';
import type {
  LinvoTextAttributes,
  LinvoTextStyleAttributes,
...
export const formatTextCommand: Command<{
  ...
  styles: LinvoTextStyleAttributes;
  mode?: 'replace' | 'merge';
}> = (ctx, next) => {
              ...
              Object.entries(styles).map(([key, value]) => {
                if (typeof value === 'boolean') {
                  return [
                    key,
                    (inlineEditor.marks &&
                      inlineEditor.marks[key as keyof LinvoTextAttributes]) ||
                    (delta &&
                      delta.attributes &&
                      delta.attributes[key as keyof LinvoTextAttributes])
                      ? null
                      : value,
                  ];
                }
