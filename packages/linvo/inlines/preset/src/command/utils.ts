...
function getCombinedFormatFromInlineEditors(
  inlineEditors: [LinvoInlineEditor, InlineRange | null][]
): LinvoTextAttributes {
  const formatArr: LinvoTextAttributes[] = [];
  ...
  return formatArr.reduce((acc, cur) => {
    const newFormat: LinvoTextAttributes = {};
    for (const key in acc) {
      const typedKey = key as keyof LinvoTextAttributes;
...
function getSelectedInlineEditors(
  ...
  filter: (
    inlineRoot: InlineRootElement<LinvoTextAttributes>
  ) => InlineEditor<LinvoTextAttributes> | []
) {
  return blocks.flatMap(el => {
    const inlineRoot = el.querySelector<
      InlineRootElement<LinvoTextAttributes>
    >(`[${INLINE_ROOT_ATTR}]`);
...
function handleCurrentSelection(
  ...
  handler: (
    ...
    inlineEditors: InlineEditor<LinvoTextAttributes>[]
  ) => { textAttributes: LinvoTextAttributes } | boolean | void
): Chain<InitCommandCtx & { textAttributes: LinvoTextAttributes }> {
  return chain.try(chain => [
    ...
    chain.pipe((ctx, next) => {
      const selectedInlineEditors = Array.from<InlineRootElement>(
        ...
        .map((el): LinvoInlineEditor => el.inlineEditor);
