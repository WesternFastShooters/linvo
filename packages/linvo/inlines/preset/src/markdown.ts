import { LatexExtension } from '@linvo/linvo-inline-latex';
...
export const BoldItalicMarkdown = InlineMarkdownExtension<LinvoTextAttributes>(
  {
    name: 'bolditalic',
...
export const BoldMarkdown = InlineMarkdownExtension<LinvoTextAttributes>({
  name: 'bold',
...
export const ItalicExtension = InlineMarkdownExtension<LinvoTextAttributes>({
  name: 'italic',
...
export const CodeExtension = InlineMarkdownExtension<LinvoTextAttributes>({
  name: 'code',
  pattern: /.*`([^\s][^`]*[^\s])`\s$|.*`([^\s`])`\s$/,
  action: ({ inlineEditor, prefixText, inlineRange, pattern, undoManager }) => {
    const match = prefixText.match(pattern);
