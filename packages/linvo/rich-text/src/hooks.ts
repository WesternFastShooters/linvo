...
import { isStrictUrl } from '@linvo/linvo-shared/utils';
import type {
  BeforeinputHookCtx,
...
const autoIdentifyLink = (ctx: HookContext<LinvoTextAttributes>) => {
...
function handleExtendedAttributes(
  ctx:
    | BeforeinputHookCtx<LinvoTextAttributes>
    | CompositionEndHookCtx<LinvoTextAttributes>
) {
...
export const onVBeforeinput = (
  ctx: BeforeinputHookCtx<LinvoTextAttributes>
) => {
...
export const onVCompositionEnd = (
  ctx: CompositionEndHookCtx<LinvoTextAttributes>
) => {
  handleExtendedAttributes(ctx);
};
