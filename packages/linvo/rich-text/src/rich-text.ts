...
export class RichText extends WithDisposable(ShadowlessElement) {
  ...
  private readonly _inlineEditor$ = signal<LinvoInlineEditor | null>(null);
  ...
  private _init() {
    ...
    // init inline editor
    this._inlineEditor$.value = new InlineEditor<LinvoTextAttributes>(
      this._yText,
      {
  ...
  accessor embedChecker: <
    TextAttributes extends LinvoTextAttributes = LinvoTextAttributes,
  >(
  ...
  @property({ attribute: false })
  accessor markdownMatches: InlineMarkdownMatch<LinvoTextAttributes>[] = [];
