...
class PasteTr {
  ...
  focusPasted = () => {
    ...
    const cursorBlock =
      this.pointState.model.flavour === 'linvo:code' || !this.lastSnapshot
  ...
  constructor(
    ...
    this.firstSnapshotIsPlainText =
      this.firstSnapshot.flavour === 'linvo:paragraph' &&
  ...
  private _transformLinkDelta(
    ...
    const newDelta = delta.map(op => {
      ...
      const reference: LinvoTextAttributes['reference'] = {
        pageId,
        type: 'LinkedPage',
      };
      // Title alias
  ...
  merge() {
    ...
    if (this.pointState.model.flavour === 'linvo:code') {
      this._mergeCode();
      return;
    }
