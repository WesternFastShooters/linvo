...
export class DNDAPIExtension extends Extension {
  ...
  fromEntity(options: {
    ...
    const { docId, flavour = 'linvo:embed-linked-doc', blockId } = options;
    ...
    const props = {
      ...
      ...(blockId ? { blockId } : {}),
      pageId: docId,
      style: flavour === 'linvo:embed-synced-doc' ? 'syncedDoc' : 'vertical',
    };
    return {
      ...snapshot,
      content: [
