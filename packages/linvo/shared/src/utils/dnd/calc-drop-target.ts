import { DatabaseBlockModel, ListBlockModel } from '@linvo/linvo-model';
import { type Point, Rect } from '@linvo/global/gfx';
...
export function calcDropTarget(
  ...
  const schema = model.store.schema.get('linvo:database');
  ...
  if (!shouldAppendToDatabase && !matchModels(model, [DatabaseBlockModel])) {
    const databaseBlockComponent =
      element.closest<BlockComponent>('linvo-database');
