...
import type { Point } from '@linvo/global/gfx';
import { BLOCK_ID_ATTR } from '@linvo/std';
...
export function getDropRectByPoint(
  ...
  if (isDatabase) {
    ...
    for (; i < len; i++) {
      ...
      if (e.classList.contains('linvo-database-block-row-cell-content')) {
      ...
      if (e.classList.contains('linvo-database-block-row')) {
  ...
  if (parent?.classList.contains('linvo-database-block-row-cell-content')) {
    result.flag = DropFlags.Database;
...
function getDatabaseBlockTableElement(element: Element) {
  return element.querySelector('.linvo-database-block-table');
}
...
function getDatabaseBlockColumnHeaderElement(element: Element) {
  return element.querySelector('.linvo-database-column-header');
}
...
function getCellRect(element: Element, bounds?: DOMRect) {
  if (!bounds) {
    const table = element.closest('.linvo-database-block-table');
    if (!table) {
      return null;
    }
    bounds = table.getBoundingClientRect();
  }
  // linvo-database-block-row-cell
