import { EdgelessCRUDIdentifier } from '@linvo/linvo-block-surface';
...
import type { IVec, Point, Rect } from '@linvo/global/gfx';
import { type BlockComponent, WidgetComponent } from '@linvo/std';
...
import type { LINVO_DRAG_HANDLE_WIDGET } from './consts.js';
...
export class LinvoDragHandleWidget extends WidgetComponent<RootBlockModel> {
  ...
  override render() {
    ...
    const classes = {
      'linvo-drag-handle-grabber': true,
      dots: isGfx ? true : false,
    };

    return html`
      <div class="linvo-drag-handle-widget">
        <div class="linvo-drag-handle-container">
        ...
        <div class="linvo-drag-hover-rect" style=${hoverRectStyle}></div>
      </div>
    `;
  }

  @query('.linvo-drag-handle-container')
  accessor dragHandleContainer!: HTMLDivElement;

  @query('.linvo-drag-handle-grabber')
...
declare global {
  interface HTMLElementTagNameMap {
    [LINVO_DRAG_HANDLE_WIDGET]: LinvoDragHandleWidget;
  }
}
