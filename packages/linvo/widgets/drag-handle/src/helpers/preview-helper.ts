...
import { BlockStdScope, BlockViewIdentifier } from '@linvo/std';
import type {
  BlockModel,
  BlockViewType,
  ExtensionType,
  ...
  getPreviewStd = (blockIds: string[]) => {
    ...
    const extensions = [
      ...
      EditorSettingExtension({ setting$: settingSignal }),
      {
        setup(di) {
          di.override(
            BlockViewIdentifier('linvo:database'),
            () => literal`linvo-dnd-preview-database`
          );
        },
      } as ExtensionType,
      {
        setup(di) {
          di.override(BlockViewIdentifier('linvo:image'), () => {
            return (model: BlockModel) => {
              const parent = model.store.getParent(model.id);

              if (parent?.flavour === 'linvo:surface') {
                return literal`linvo-edgeless-placeholder-preview-image`;
              }

              return literal`linvo-placeholder-preview-image`;
            };
          });
        },
      } as ExtensionType,
    ...
    const noteBlock = this.widget.host.querySelector('linvo-note');
