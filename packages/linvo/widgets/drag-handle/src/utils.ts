import { type CalloutBlockComponent } from '@linvo/linvo-block-callout';
import {
  LINVO_EDGELESS_NOTE,
  EdgelessNoteBackground,
  EdgelessNoteBlockComponent,
} from '@linvo/linvo-block-note';
...
import type { BlockComponent, EditorHost } from '@linvo/std';
import type {
  BaseSelection,
  BlockModel,
  BlockSnapshot,
...
export const insideDatabaseTable = (element: Element) => {
  return !!element.closest('.linvo-database-block-table');
};
...
export const getParentNoteBlock = (blockComponent: BlockComponent) => {
  return blockComponent.closest('linvo-note, linvo-edgeless-note') ?? null;
};
...
export const getClosestBlockByPoint = (
  ...
  if (!closestNoteBlock || closestNoteBlock.closest('.linvo-surface-ref')) {
    return null;
  }
  ...
  const blockSelector =
    '.linvo-note-block-container > .linvo-block-children-container > [data-block-id]';
  ...
  if (matchModels(closestBlock.model, [ParagraphBlockModel])) {
    const callout =
      closestBlock.closest<CalloutBlockComponent>('linvo-callout');
    if (callout) {
      return callout;
    }
  }
...
function getHoveringNote(point: Point) {
  ...
  for (const el of elements) {
    ...
    if (el instanceof EdgelessNoteBackground) {
      return el.closest(LINVO_EDGELESS_NOTE) ?? null;
    }
  }
  return null;
}
