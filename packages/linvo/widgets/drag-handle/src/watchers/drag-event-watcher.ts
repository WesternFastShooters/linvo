...
import { ParagraphBlockComponent } from '@linvo/linvo-block-paragraph';
import { DropIndicator } from '@linvo/linvo-components/drop-indicator';
import {
...
import { surfaceRefToEmbed } from '@linvo/linvo-shared/adapters';
import {
...
export type DragBlockPayload = DragPayload<DragBlockEntity, DragFromLinvo>;

declare module '@linvo/std' {
  interface DNDEntity {
...
export class DragEventWatcher {
    ...
    if (edge === 'right' && matchModels(dropModel, [ListBlockModel])) {
      ...
      if (
        snapshot.content.every(block =>
          schema.safeValidate(block.flavour, 'linvo:list')
        )
      ) {
      ...
      const placement =
        isDropOnNoteBlock &&
        schema.safeValidate(snapshot.content[0].flavour, 'linvo:note')
          ? 'in'
          : edge === 'top'
            ? 'before'
            : 'after';
  ...
  private readonly _onPageDrop = (
    ...
    if (
      ...
      snapshot.content.every(block => block.flavour === 'linvo:note')
    ) {
    ...
    if (
      ...
      if (dragPayload.from?.docId === this.widget.store.id) {
        ...
        const walk = (block: BlockSnapshot) => {
          if (block.flavour === 'linvo:surface') {
            Object.values(
  ...
  private readonly _rewriteSnapshotXYWH = (
    ...
    const rewrite = (block: BlockSnapshot) => {
      if (block.flavour === 'linvo:surface') {
        ...
        if (block.flavour === 'linvo:embed-iframe') {
          ...
          block.flavour === 'linvo:attachment' ||
          block.flavour === 'linvo:bookmark' ||
          block.flavour.startsWith('linvo:embed-')
        ) {
  ...
  private readonly _dropAsGfxBlock = (
    ...
    if (!groupByParent.empty?.length) {
      ...
      if (groupByParent['linvo:surface']) {
        const content = groupByParent['linvo:surface'];
      ...
      const content = snapshot.content.filter(block =>
        schema.safeValidate(block.flavour, 'linvo:note')
      );
  ...
  private async _dropToModel(
    ...
    try {
      ...
      if (snapshot.content.length === 1) {
        const [first] = snapshot.content;
        if (first.flavour === 'linvo:embed-linked-doc') {
