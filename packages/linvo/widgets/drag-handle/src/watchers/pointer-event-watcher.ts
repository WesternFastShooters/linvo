import type { NoteBlockComponent } from '@linvo/linvo-block-note';
import { captureEventTarget } from '@linvo/linvo-shared/utils';
...
import { LINVO_DRAG_HANDLE_WIDGET } from '../consts.js';
...
export class PointerEventWatcher {
  ...
  private readonly _clickHandler: UIEventHandler = ctx => {
    ...
    const insideDragHandle = !!element?.closest(LINVO_DRAG_HANDLE_WIDGET);
  ...
  private readonly _pointerOutHandler: UIEventHandler = ctx => {
    ...
    const inPage = !!relatedElement?.closest('.linvo-page-viewport');

    const inDragHandle = !!relatedElement?.closest(LINVO_DRAG_HANDLE_WIDGET);
  ...
  private readonly _throttledPointerMoveHandler = throttle<UIEventHandler>(
    ctx => {
      ...
      if (element.closest('.linvo-drag-handle-container')) return;
