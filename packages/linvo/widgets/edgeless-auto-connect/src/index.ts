...
import { SmallDocIcon } from '@linvo/linvo-components/icons';
import {
  FrameBlockModel,
  NoteBlockModel,
  NoteDisplayMode,
...
import { matchModels, stopPropagation } from '@linvo/linvo-shared/utils';
...
export const LINVO_EDGELESS_AUTO_CONNECT_WIDGET =
  'linvo-edgeless-auto-connect-widget';
...
export class EdgelessAutoConnectWidget extends WidgetComponent<RootBlockModel> {
    ...
    return html`${repeat(
      ...
      note => {
        ...
        const style = styleMap({
          ...
          borderRadius: '50%',
          backgroundColor: 'var(--linvo-text-secondary-color)',
        ...
        return html`<div style=${style} class="edgeless-only-index-label">
          ...
          <linvo-tooltip tip-position="bottom">
  ...
  private _initLabels() {
    ...
    const surfaceRefs = this.block.std.store
      .getBlocksByFlavour('linvo:surface-ref')
    ...
    this._disposables.add(
      this.store.slots.blockUpdated.subscribe(payload => {
        if (payload.flavour === 'linvo:surface-ref') {
          switch (payload.type) {
            case 'add':
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-edgeless-auto-connect-widget': EdgelessAutoConnectWidget;
  }
}
