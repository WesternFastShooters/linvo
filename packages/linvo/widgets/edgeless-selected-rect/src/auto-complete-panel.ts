import { EdgelessFrameManagerIdentifier } from '@linvo/linvo-block-frame';
import {
  CanvasElementType,
...
import { FontFamilyIcon } from '@linvo/linvo-components/icons';
import {
  mountShapeTextEditor,
  SHAPE_OVERLAY_HEIGHT,
  SHAPE_OVERLAY_WIDTH,
  ShapeComponentConfig,
} from '@linvo/linvo-gfx-shape';
import {
  insertEdgelessTextCommand,
  mountTextElementEditor,
} from '@linvo/linvo-gfx-text';
import type {
  Connection,
...
import type { XYWH } from '@linvo/global/gfx';
import {
  Bound,
  clamp,
  normalizeDegAngle,
...
export class EdgelessAutoCompletePanel extends WithDisposable(LitElement) {
  static override styles = css`
    ...
    .row-button {
      ...
      font-style: normal;
      font-weight: 500;
      border: 1px solid var(--linvo-border-color, #e3e2e4);
  ...
  private _addFrame() {
    ...
    const props = this.std.get(EditPropsStore).applyLastProps('linvo:frame', {
      title: new Y.Text(`Frame ${frameIndex}`),
      xywh: serializeXYWH(...xywh),
      presentationIndex: frameMgr.generatePresentationIndex(),
    });
    const id = this.crud.addBlock('linvo:frame', props, surfaceBlockModel);
  ...
  private _addNote() {
    ...
    store.addBlock('linvo:paragraph', { type: 'text' }, id);
  ...
  private _showFrameOverlay() {
    ...
    const strokeColor = this.std
      .get(ThemeProvider)
      .getCssVariableColor('--linvo-black-30');
  ...
  private _showNoteOverlay() {
    ...
    const background = this.edgeless.std
      .get(ThemeProvider)
      .getColorValue(
        this.edgeless.std.get(EditPropsStore).lastProps$.value['linvo:note']
          .background,
