import { addSiblingAttachmentBlocks } from '@linvo/linvo-block-attachment';
import { insertDatabaseBlockCommand } from '@linvo/linvo-block-database';
import { insertEmptyEmbedIframeCommand } from '@linvo/linvo-block-embed';
import { insertImagesCommand } from '@linvo/linvo-block-image';
import { insertLatexBlockCommand } from '@linvo/linvo-block-latex';
import {
  canDedentListCommand,
  canIndentListCommand,
  dedentListCommand,
  indentListCommand,
} from '@linvo/linvo-block-list';
import { updateBlockType } from '@linvo/linvo-block-note';
import {
  canDedentParagraphCommand,
  canIndentParagraphCommand,
  dedentParagraphCommand,
  indentParagraphCommand,
} from '@linvo/linvo-block-paragraph';
import { DefaultTool, getSurfaceBlock } from '@linvo/linvo-block-surface';
import { insertSurfaceRefBlockCommand } from '@linvo/linvo-block-surface-ref';
import { toggleEmbedCardCreateModal } from '@linvo/linvo-components/embed-card-modal';
import { toast } from '@linvo/linvo-components/toast';
import { insertInlineLatex } from '@linvo/linvo-inline-latex';
import { toggleLink } from '@linvo/linvo-inline-link';
import {
  formatBlockCommand,
  formatNativeCommand,
  formatTextCommand,
...
import { insertContent } from '@linvo/linvo-rich-text';
import {
...
import type { LinvoTextStyleAttributes } from '@linvo/linvo-shared/types';
import {
  createDefaultDoc,
  openSingleFileWith,
  type Signal,
} from '@linvo/linvo-shared/utils';
import type { LinvoLinkedDocWidget } from '@linvo/linvo-widget-linked-doc';
...
const textToolActionItems: KeyboardToolbarActionItem[] = [
  {
    ...
    icon: TextIcon(),
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('linvo:paragraph'),
    action: ({ std }) => {
  ...
  {
    name: 'CodeBlock',
    showWhen: ({ std }) => std.store.schema.flavourSchemaMap.has('linvo:code'),
    icon: CodeBlockIcon(),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
        flavour: 'linvo:code',
  ...
  },
  {
    ...
    icon: DividerIcon(),
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('linvo:divider'),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
...
const listToolActionItems: KeyboardToolbarActionItem[] = [
  {
    ...
    showWhen: ({ std }) => std.store.schema.flavourSchemaMap.has('linvo:list'),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
        flavour: 'linvo:list',
        props: {
  ...
  items: [
    {
      ...
      icon: NewPageIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('linvo:embed-linked-doc'),
      action: ({ std }) => {
        std.command
          .chain()
    ...
    },
    {
      ...
      showWhen: ({ std, rootComponent }) => {
        const linkedDocWidget = std.view.getWidget(
          'linvo-linked-doc-widget',
          rootComponent.model.id
        );
        if (!linkedDocWidget) return false;

        return std.store.schema.flavourSchemaMap.has('linvo:embed-linked-doc');
      },
      action: ({ rootComponent, closeToolPanel }) => {
        ...
        assertType<LinvoLinkedDocWidget>(linkedDocWidget);
  ...
  items: [
    {
      ...
      icon: ImageIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('linvo:image'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
    ...
    },
    {
      ...
      icon: LinkIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('linvo:bookmark'),
      action: async ({ std }) => {
    ...
    },
    {
      ...
      icon: TeXIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('linvo:latex'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
  ...
  items: [
    {
      ...
      showWhen: ({ std }) => {
        return std.store.schema.flavourSchemaMap.has('linvo:embed-iframe');
      },
      action: async ({ std }) => {
        std.command
          .chain()
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('linvo:embed-youtube'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('linvo:embed-github'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('linvo:embed-figma'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('linvo:embed-loom'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
...
const documentGroupFrameToolGroup: DynamicKeyboardToolPanelGroup = ({
  ...
  const frameModels = store
    .getBlocksByFlavour('linvo:frame')
  ...
  items: [
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('linvo:database'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
        ...
        ...(
          ...
          action: ({ std }) => {
            const payload = {
              ...
              } satisfies LinvoTextStyleAttributes,
            };
            std.command
              .chain()
              .try(chain => [
...
export const defaultKeyboardToolbarConfig: KeyboardToolbarConfig = {
  items: [
    moreToolPanel,
    // TODO(@L-Sun): add the AI action on the product side.
