...
import { ViewportElementProvider } from '@linvo/linvo-shared/services';
import {
  autoScroll,
  getScrollContainer,
...
export const LINVO_PAGE_DRAGGING_AREA_WIDGET =
  'linvo-page-dragging-area-widget';

export class LinvoPageDraggingAreaWidget extends WidgetComponent<RootBlockModel> {
  static excludeFlavours: string[] = ['linvo:note', 'linvo:surface'];
    ...
    const getAllNodeFromTree = (): BlockComponent[] => {
      ...
      this.host.view.walkThrough(node => {
        ...
        if (
          ...
          !LinvoPageDraggingAreaWidget.excludeFlavours.includes(
            view.model.flavour
          )
        ) {
          blocks.push(view);
  ...
  override render() {
    ...
    return html`
      ...
      <div class="linvo-page-dragging-area" style=${styleMap(style)}></div>
    `;
  }
...
declare global {
  interface HTMLElementTagNameMap {
    [LINVO_PAGE_DRAGGING_AREA_WIDGET]: LinvoPageDraggingAreaWidget;
  }
}
