import { getInlineEditorByModel } from '@linvo/linvo-rich-text';
import type { LinvoInlineEditor } from '@linvo/linvo-shared/types';
import { DisposableGroup } from '@linvo/global/disposable';
import type { UIEventStateContext } from '@linvo/std';
import { TextSelection, WidgetComponent } from '@linvo/std';
import { InlineEditor } from '@linvo/std/inline';
import debounce from 'lodash-es/debounce';

import { LINVO_SLASH_MENU_TRIGGER_KEY } from './consts';
...
export class LinvoSlashMenuWidget extends WidgetComponent {
  ...
  private readonly _handleInput = (
    ...
    inlineRangeApplyCallback(() => {
      ...
      if (!text.endsWith(LINVO_SLASH_MENU_TRIGGER_KEY)) return;
  ...
  private readonly _onCompositionEnd = (ctx: UIEventStateContext) => {
    ...
    if (event.data !== LINVO_SLASH_MENU_TRIGGER_KEY) return;
  ...
  private readonly _onKeyDown = (ctx: UIEventStateContext) => {
    ...
    if (event.isComposing || key !== LINVO_SLASH_MENU_TRIGGER_KEY) return;
