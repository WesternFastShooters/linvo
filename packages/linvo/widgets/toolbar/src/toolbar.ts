import { DatabaseSelection } from '@linvo/linvo-block-database';
import { EdgelessLegacySlotIdentifier } from '@linvo/linvo-block-surface';
import { TableSelection } from '@linvo/linvo-block-table';
import {
  darkToolbarStyles,
...
export const LINVO_TOOLBAR_WIDGET = 'linvo-toolbar-widget';

export class LinvoToolbarWidget extends WidgetComponent {
  ...
  updateWithSurface(
    ...
    if (activated && surface) {
      ...
      if (hasLocked) {
        flavour = 'linvo:surface:locked';
      } else {
        if (paired.length === 1) {
          flavour = paired[0][0];
          if (flavour === 'linvo:surface:shape' && paired[0][1].length === 1) {
      ...
      if (!sideOptions) {
        ...
        if (flavours.has('linvo:surface:frame')) {
          sideOptions = sideMap.get('linvo:surface:frame') ?? null;
        } else if (flavours.has('linvo:surface:group')) {
          sideOptions = sideMap.get('linvo:surface:group') ?? null;
        }
      }
    }
    ...
    disposables.add(
      std.selection.find$(TextSelection).subscribe(result => {
        ...
        batch(() => {
          ...
          flavour$.value = 'linvo:note';
          placement$.value = toolbarRegistry.getModulePlacement('linvo:note');
      ...
      std.selection.filter$(BlockSelection).subscribe(selections => {
        ...
        batch(() => {
          ...
          placement$.value = toolbarRegistry.getModulePlacement(
            flavour,
            flavour === 'linvo:note' ? 'top' : 'top-start'
          );
