...
export const sideMap = new Map([
  ...
  ['linvo:surface:frame', { top: 28 }],
  // includes group element
  ['linvo:surface:group', { top: 20 }],
  // has only one shape element
  ['linvo:surface:shape', { top: 26, bottom: -26 }],
]);
...
export function autoUpdatePosition(
  ...
  const isInline = flavour === 'linvo:note';
...
export function renderToolbar(
  ...
  const actions = [
    flavour,
    `custom:${flavour}`,
    hasSurfaceScope ? ['linvo:surface:*', 'custom:linvo:surface:*'] : [],
    'linvo:*',
    'custom:linvo:*',
  ]
    .flat()
  ...
  // Avoids shaking
  if (flavour === 'linvo:note' && context.std.range.value) {
