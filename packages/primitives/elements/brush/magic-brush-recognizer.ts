...
import {
  MAGIC_BRUSH_MIN_BOUND_SIZE,
  MAGIC_BRUSH_MIN_PATH_LENGTH,
  MAGIC_BRUSH_MIN_SCORE,
...
const ORIGIN: PointCloudPoint = { x: 0, y: 0, id: 0 };
...
const MAGIC_BRUSH_TEMPLATES: PointCloudTemplate[] = [
  createTemplate(
    'rectangle',
    polylineToStrokes(
      [
...
export function recognizeMagicBrush(
  strokes: readonly (readonly IVec[])[]
): MagicBrushMatch | null {
  ...
  if (
    Math.max(bounds.w, bounds.h) < MAGIC_BRUSH_MIN_BOUND_SIZE ||
...
function polylineToStrokes(points: IVec[], close = false): IVec[][] {
  const stroke = samplePolyline(points, close ? 14 : 12);
  return [stroke];
}

function samplePolyline(points: IVec[], samplesPerSegment: number): IVec[] {
  const sampled: IVec[] = [];
...
function sampleEllipse(width: number, height: number, count = 48): IVec[] {
  const rx = width / 2;
  const ry = height / 2;
  const cx = rx + 10;
  const cy = ry + 10;
  ...
  return Array.from({ length: count + 1 }, (_, index) => {
    ...
    return [cx + rx * Math.cos(angle), cy + ry * Math.sin(angle)] as IVec;
  });
}

function sampleLine(start: IVec, end: IVec, count = 12): IVec[] {
  return Array.from({ length: count + 1 }, (_, index) => {
    const t = index / count;
    return [
      start[0] + (end[0] - start[0]) * t,
...
function normalizeStrokes(strokes: readonly (readonly IVec[])[]): IVec[][] {
  return strokes
    .map(stroke =>
      stroke
        .filter(
...
function flattenPoints(strokes: readonly IVec[][]): PointCloudPoint[] {
...
function normalizePointCloud(points: PointCloudPoint[]): PointCloudPoint[] {
...
function resample(
  ...
  const result: PointCloudPoint[] = [{ ...points[0] }];
...
function scale(points: PointCloudPoint[]): PointCloudPoint[] {
...
function getCentroid(points: PointCloudPoint[]): PointCloudPoint {
...
function greedyCloudMatch(
  points: PointCloudPoint[],
  template: PointCloudPoint[]
): number {
  const epsilon = 0.5;
  const step = Math.max(
    1,
    Math.floor(Math.pow(points.length, 1 - epsilon))
  );
...
function cloudDistance(
  left: PointCloudPoint[],
  right: PointCloudPoint[],
  start: number
): number {
  ...
  do {
    ...
    const weight =
      1 - ((index - start + left.length) % left.length) / left.length;
...
function distance(left: PointCloudPoint, right: PointCloudPoint): number {
  const dx = right.x - left.x;
  const dy = right.y - left.y;
  return Math.hypot(dx, dy);
}

function getPathLength(points: PointCloudPoint[]): number {
  let total = 0;
...
function getPathLengthFromStrokes(strokes: readonly IVec[][]): number {
  let total = 0;
...
function getBounds(strokes: readonly IVec[][]): MagicBrushBounds {
...
function getBoundsFromPointCloud(points: PointCloudPoint[]): MagicBrushBounds {
  let minX = Number.POSITIVE_INFINITY;
  let minY = Number.POSITIVE_INFINITY;
  let maxX = Number.NEGATIVE_INFINITY;
  let maxY = Number.NEGATIVE_INFINITY;
...
function mapKindToShape(kind: Exclude<MagicBrushKind, 'arrow-left' | 'arrow-right'>) {
  switch (kind) {
    case 'rectangle':
