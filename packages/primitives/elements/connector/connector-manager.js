import { AStarRunner, Overlay } from '@linvo-core/block-surface';
import { ConnectorElementModel, ConnectorMode, GroupElementModel, } from '@linvo-core/content';
import { ThemeProvider } from '@linvo-core/shared/services';
import { LinvoError } from '@linvo-core/global/exceptions';
import { almostEqual, Bound, getBezierCurveBoundingBox, getBezierParameters, getBoundFromPoints, getBoundWithRotation, getPointFromBoundsWithRotation, isOverlap, isVecZero, lineIntersects, PI2, PointLocation, sign, toRadian, Vec, } from '@linvo-core/global/gfx';
...
function getConnectablePoints(startPoint, endPoint, nextStartPoint, lastEndPoint, startBound, endBound, expandStartBound, expandEndBound) {
    ...
    if (!startEnds[0] || !startEnds[1]) {
        throw new LinvoError(LinvoError.ErrorCode.ValueNotExists, 'Failed to get start and end points when getting connectable points');
    }
    ...
    if (almostEqual(bound.x, result[0]))
        ...
        if (slope === 0) {
            if (xDirection > 0) {
                ...
                if (!intersects) {
                    throw new LinvoError(LinvoError.ErrorCode.ValueNotExists, 'Failed to get line intersections for getNextPoint');
                }
...
export class ConnectionOverlay extends Overlay {
    ...
    _getEmphasisColor() {
        return getComputedStyle(this.gfx.std.host).getPropertyValue('--linvo-text-emphasis-color');
    }
    _setupThemeListener() {
