...
import { almostEqual } from '@linvo-core/shared/utils';
import { LinvoError, ErrorCode } from '@linvo-core/global/exceptions';
import { Bound, type IVec, Vec } from '@linvo-core/global/gfx';
...
export class EdgelessConnectorLabelEditor extends WithDisposable(
  ...
  static override styles = css`
    .edgeless-connector-label-editor {
      ...
      border: ${BORDER_WIDTH}px solid var(--linvo-primary-color, #1e96eb);
      background: var(--linvo-background-primary-color, #fff);
      border-radius: 2px;
  ...
  override firstUpdated() {
    ...
    this._resizeObserver.observe(this.plainTextEditor);
        ...
        this.disposables.add(() => {
          if (connector.text) {
            ...
            if (len === 0) {
              ...
              bindSurfaceText(connector.text)?.set(trimed);
            }
          }
  ...
  override render() {
    ...
    const textController = bindSurfaceText(connector.text);
