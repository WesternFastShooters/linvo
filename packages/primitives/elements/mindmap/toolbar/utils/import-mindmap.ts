import { openSingleFileWith } from '@linvo-core/shared/utils';
...
export async function importMindmap(bound: Bound): Promise<MindMapNode> {
  ...
  if (!file) {
    throw new LinvoError(ErrorCode.UserAbortError, 'Aborted by user');
  }
  ...
  if (file.name.endsWith('.mm')) {
    ...
    throw new LinvoError(ErrorCode.ParsingError, 'Unsupported file type');
  }
