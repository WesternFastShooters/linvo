import { shapeMethods, ShapeType } from '@linvo-core/content';
...
export function isPresetShapeType(type) {
    return PRESET_SHAPE_TYPES.includes(type);
}
function getRenderBound(xywh, strokeWidth = 0) {
    const [x, y, w, h] = xywh;
    const inset = Math.max(strokeWidth, 0) / 2;
    return new Bound(x + inset, y + inset, Math.max(1, w - inset * 2), Math.max(1, h - inset * 2));
}
export function getPresetShapePoints(type, xywh, strokeWidth = 0) {
...
function getSubroutineLines(bound) {
...
function getInnerEllipse(bound, scale = 0.72) {
    return {
...
function getHorizontalCylinderInnerArc(bound) {
...
function getCylinderEllipse(bound) {
...
function drawRoughLines(rc, lines, options) {
    for (const [start, end] of lines) {
        rc.line(start[0], start[1], end[0], end[1], options);
    }
}
export function drawPresetShapeUnderlay(ctx, type, xywh, strokeWidth = 0) {
    const [x, y, w, h] = xywh;
    if (type === ShapeType.MultiDocument) {
        shapeMethods.document.draw(ctx, getRenderBound([x - 12, y - 10, w, h], strokeWidth));
        ctx.stroke();
        shapeMethods.document.draw(ctx, getRenderBound([x - 6, y - 5, w, h], strokeWidth));
        ctx.stroke();
    }
...
export function drawPresetShapeUnderlayRough(rc, type, xywh, options) {
    const [x, y, w, h] = xywh;
    if (type === ShapeType.MultiDocument) {
        rc.polygon(getPresetShapePoints(ShapeType.Document, [x - 12, y - 10, w, h]), {
            ...options,
            fill: undefined,
        });
    ...
    if (type === ShapeType.StackedRect) {
        for (const [dx, dy] of [
            ...
            rc.polygon(shapeMethods.rect.points(getRenderBound([x + dx, y + dy, w, h])), {
                ...options,
                fill: undefined,
            });
        }
...
export function drawPresetShapeOverlay(ctx, type, xywh, strokeWidth = 0) {
    ...
    switch (type) {
        ...
        case ShapeType.Bang: {
            ...
            ctx.beginPath();
            ctx.ellipse(bound.x + bound.w / 2, bound.y + bound.h * 0.78, radius, radius, 0, 0, Math.PI * 2);
            ctx.fill();
            break;
        }
...
export function drawPresetShapeOverlayRough(rc, type, xywh, options) {
