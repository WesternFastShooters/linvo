import { MAX_IMAGE_WIDTH, } from '@linvo-core/content';
import { EMBED_CARD_HEIGHT, EMBED_CARD_WIDTH, } from '@linvo-core/shared/consts';
import { FileSizeLimitProvider } from '@linvo-core/shared/services';
import { readImageSize, transformModel, withTempBlobData, } from '@linvo-core/shared/utils';
...
// Single embed config.
export const AttachmentEmbedConfigIdentifier = createIdentifier('LinvoAttachmentEmbedConfigIdentifier');
...
// A embed config map.
export const AttachmentEmbedConfigMapIdentifier = createIdentifier('LinvoAttachmentEmbedConfigMapIdentifier');
export const AttachmentEmbedProvider = createIdentifier('LinvoAttachmentEmbedProvider');
...
const embedConfig = [
    {
        ...
        check: model => model.store.schema.flavourSchemaMap.has('linvo:image') &&
    ...
    },
    {
        ...
        <iframe
          ...
          credentialless
        ></iframe>
        <div class="linvo-attachment-embed-event-mask"></div>
      `;
        },
    },
    {
...
async function turnIntoImageBlock(model) {
    if (!model.store.schema.flavourSchemaMap.has('linvo:image')) {
    ...
    transformModel(model, 'linvo:image', imageProp);
}
