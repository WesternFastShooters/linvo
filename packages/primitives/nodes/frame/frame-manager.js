import { Overlay } from '@linvo-core/block-surface';
import { EditPropsStore } from '@linvo-core/shared/services';
import { DisposableGroup } from '@linvo-core/global/disposable';
import { LinvoError, ErrorCode } from '@linvo-core/global/exceptions';
import { Bound, deserializeXYWH, } from '@linvo-core/global/gfx';
import { generateKeyBetweenV2, getTopElements, GfxBlockElementModel, GfxExtension, GfxExtensionIdentifier, isGfxGroupCompatibleModel, renderableInEdgeless, } from '@linvo-core/std/gfx';
...
export class EdgelessFrameManager extends GfxExtension {
    ...
    _addFrameBlock(bound) {
        ...
        const props = this.gfx.std
            ...
            title: new Text(`Frame ${this.frames.length + 1}`),
            xywh: bound.serialize(),
            index: this.gfx.layer.generateIndex(true),
            presentationIndex: this.generatePresentationIndex(),
        });
        const id = this.gfx.doc.addBlock('linvo:frame', props, surfaceModel);
        const frameModel = this.gfx.getElementById(id);
        if (!frameModel || !isFrameBlock(frameModel)) {
            throw new LinvoError(ErrorCode.GfxBlockElementError, 'Frame model is not found');
        }
        return frameModel;
    }
    _watchElementAdded() {
...
export function isFrameBlock(element) {
    return !!element && element.flavour === 'linvo:frame';
}
