...
import type { BlockComponent, UIEventStateContext } from '@linvo-core/std';
import {
  BlockSelection,
  ShadowlessElement,
  TextSelection,
} from '@linvo-core/std';
import type { BaseSelection } from '@linvo-core/store';
...
export class ImageBlockPageComponent extends SignalWatcher(
  ...
  static override styles = css`
    ...
    linvo-page-image .linvo-image-status {
      position: absolute;
      left: 18px;
      bottom: 18px;
    }

    linvo-page-image .resizable-img {
      position: relative;
      max-width: 100%;
    }

    linvo-page-image .resizable-img img {
      width: 100%;
      height: 100%;
    }

    linvo-page-image .comment-highlighted {
  ...
  override render() {
    ...
    return html`
      ...
      ${when(
        ...
        () =>
          html`<linvo-resource-status
            class="linvo-image-status"
            .message=${description}
...
declare global {
  interface HTMLElementTagNameMap {
    'linvo-page-image': ImageBlockPageComponent;
  }
}
