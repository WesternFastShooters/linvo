...
import { COLORS, FIRST_COLOR } from './consts';
import type {
  Hsv,
  Hsva,
  ModeType,
...
export const parseStringToRgba = (value: string) => {
  ...
  // Compatible old format: `--linvo-palette-transparent`
 ...
 *
 * ```json
 * { 'fillColor': '--linvo-palette-shape-yellow' }
 * { 'fillColor': '#ffffff' }
...
export const calcCustomButtonStyle = (
  ...
  if (color.startsWith('--')) {
    if (!color.endsWith('transparent')) {
      b = 'var(--linvo-background-overlay-panel-color)';
      c = keepColor(
        rgbaToHex8(
