...
import { unsafeCSSVar, unsafeCSSVarV2 } from '@linvo-core/shared/theme';
import { IS_MOBILE } from '@linvo-core/global/env';
import { SignalWatcher, WithDisposable } from '@linvo-core/global/lit';
import { ArrowLeftBigIcon, ArrowLeftSmallIcon, CloseIcon, SearchIcon, } from '@icons/lit';
  ...
  `; }
    ...
    `;
        ...
        renderSearch() {
            ...
            return html ` <div style=${searchStyle} class="linvo-menu-search-container">
      <div
        style="font-size:20px;display:flex;align-items:center;color: var(--linvo-text-secondary-color)"
      >
        ${SearchIcon()}
      </div>
      <input
        autocomplete="off"
        class="linvo-menu-search"
        ...
        ${title.onBack
            ...
            >
              ${ArrowLeftBigIcon()}
                ...
                : nothing}
        <div
          style="flex:1;font-weight:500;font-size: 14px;line-height: 22px;color: var(--linvo-text-primary-color)"
        >
          ${title.text}
        </div>
        ${title.postfix?.()}
...
export const createModal = (container = document.body) => {
    ...
    div.style.fontFamily = 'var(--linvo-font-family)';
