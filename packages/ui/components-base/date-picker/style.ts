...
export const datePickerStyle = css`
  ...
  .popup.date-picker {
    ...
    border-radius: 12px;
    box-shadow: var(--linvo-menu-shadow);
  }
  ...
  .date-picker-small-action:hover {
    color: var(--linvo-icon-hover-color);
    background: var(--linvo-icon-hover-background);
  }
  ...
  .interactive::before {
    opacity: 0;
    transition: none;
    box-shadow: 0 0 0 3px var(--linvo-primary-color);
  }
  ...
  .interactive:focus-visible {
    outline: none;
    outline: 1px solid var(--linvo-primary-color);
  }
  ...
  .date-picker-footer {
    margin-top: 8px;
    padding-top: 8px;
    border-top: 1px solid var(--linvo-border-color);
  }
