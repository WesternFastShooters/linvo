...
/**
 * Similar to `<linvo-portal>`, but only renders once when called.
 *
...
export function createSimplePortal({ template, container = document.body, signal = new AbortController().signal, renderOptions, shadowDom = true, identifyWrapper = true, }) {
    ...
    if (identifyWrapper) {
        portalRoot.classList.add('linvo-portal');
    }
    if (shadowDom) {
    ...
    if (!root) {
        throw new LinvoError(LinvoError.ErrorCode.ValueNotExists, 'Failed to create portal root');
    }
    let updateId = 0;
    ...
    // linvo's modal will set pointer-events: none to body
    // in order to avoid the issue that the floating element in linvo cannot be clicked
