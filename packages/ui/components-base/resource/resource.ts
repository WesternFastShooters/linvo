import type { Disposable } from '@linvo-core/global/disposable';
import type { BlobState, BlobSync } from '@linvo-core/store';
import {
  computed,
  effect,
  type ReadonlySignal,
...
export class ResourceController implements Disposable {
  ...
  subscribe(): () => void {
    return effect(() => {
      ...
      const blobState$ = this.engine?.blobState$?.(blobId) ?? null;
      if (!blobState$) return;
