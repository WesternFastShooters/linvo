...
export const LINVO_VIEWPORT_OVERLAY_WIDGET = 'linvo-viewport-overlay-widget';

export class LinvoViewportOverlayWidget extends WidgetComponent<RootBlockModel> {
  static override styles = css`
    .linvo-viewport-overlay-widget {
      position: absolute;
      top: 0;
      left: 0;
      background: transparent;
      pointer-events: none;
      z-index: calc(var(--linvo-z-index-popover) - 1);
    }

    .linvo-viewport-overlay-widget.lock {
      pointer-events: auto;
    }
  `;
  ...
  override render() {
    const classes = classMap({
      'linvo-viewport-overlay-widget': true,
...
declare global {
  interface HTMLElementTagNameMap {
    [LINVO_VIEWPORT_OVERLAY_WIDGET]: LinvoViewportOverlayWidget;
  }
}
