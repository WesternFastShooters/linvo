...
describe('di', () => {
  ...
  test('scope', () => {
    ...
    const pageScope = createScope('page', appScope);
    ...
    container.scope(appScope).add(App, [System]);
    class Page {
      name = 'page';
    ...
    container.scope(pageScope).add(Page, [System, App]);
    ...
    expect(root.get(System).appName).toEqual('linvo');
    expect(() => root.get(App)).toThrowError(ServiceNotFoundError);

    const app = container.provider(appScope, root);
    expect(app.get(App).name).toEqual('app');
    expect(app.get(System).appName).toEqual('linvo');
    expect(() => root.get(Page)).toThrowError(ServiceNotFoundError);

    const page = container.provider(pageScope, app);
    expect(page.get(Page).name).toEqual('page');
    expect(page.get(App).name).toEqual('app');
    expect(page.get(System).appName).toEqual('linvo');
