...
const rootSchemaExtension = BlockSchemaExtension(rootSchema);

const gridSchema = defineBlockSchema({
  flavour: 'linvo:grid',
  props: () => ({
...
const gridSchemaExtension = BlockSchemaExtension(gridSchema);

const gridFlatSchema = defineBlockSchema({
  flavour: 'linvo:grid-flat',
  props: internal => ({
    title: internal.Text(),
...
const gridFlatSchemaExtension = BlockSchemaExtension(gridFlatSchema);

class RootModel extends BlockModel<
  ReturnType<(typeof rootSchema)['model']['props']>
> {}
class GridModel extends BlockModel<
  ReturnType<(typeof gridSchema)['model']['props']>
> {}
class GridFlatModel extends BlockModel<
  ReturnType<(typeof gridFlatSchema)['model']['props']>
> {}
...
test('init block without props should add default props', () => {
  ...
  const blockState = docState.getMap('blockState') as BlockRecord;
  blockState.set('sys:id', '0');
  blockState.set('sys:flavour', 'linvo:root');
  blockState.set('sys:children', new Y.Array());

  const block = new Block(doc.schema, blockState, doc);
  const model = block.model as RootModel;

  expect(blockState.get('prop:count')).toBe(0);
...
describe('block model should has signal props', () => {
  test('atom', () => {
    ...
    expect(blockState.get('prop:count')).toBe(1);

    // set signal
    model.props.count$.value = 2;
    expect(model.props.count).toBe(2);
    expect(isOdd.peek()).toBe(false);
    expect(blockState.get('prop:count')).toBe(2);

    // set prop
    blockState.set('prop:count', 3);
  ...
  test('nested', () => {
    ...
    expect((blockState.get('prop:style') as Y.Map<unknown>).toJSON()).toEqual({
      color: 'red',
    });
  ...
  test('with stash and pop', () => {
    ...
    const block = new Block(doc.schema, blockState, doc, { onChange });
    const model = block.model as RootModel;
    ...
    expect(blockState.get('prop:count')).toBe(3);
    expect(model.props.count).toBe(4);

    model.pop('count');
    expect(blockState.get('prop:count')).toBe(4);
  });
});
...
test('deep sync', () => {
  ...
  const blockState = docState.getMap('blockState') as BlockRecord;
  blockState.set('sys:id', '0');
  blockState.set('sys:flavour', 'linvo:grid');
  blockState.set('sys:children', new Y.Array());
  ...
  const model = block.model as GridModel;
  ...
  const getColsMap = () => blockState.get('prop:cols') as Y.Map<unknown>;
  const getRowsArr = () => blockState.get('prop:rows') as Y.Array<unknown>;
...
describe('flat', () => {
  test('flat crud', async () => {
    ...
    const blockState = docState.getMap('blockState') as BlockRecord;
    blockState.set('sys:id', '0');
    blockState.set('sys:flavour', 'linvo:grid-flat');
    blockState.set('sys:children', new Y.Array());
    ...
    const block = new Block(doc.schema, blockState, doc, { onChange });
    const model = block.model as GridFlatModel;
    ...
    expect(blockState.get('prop:cols.a.color')).toBe('red');
    expect(blockState.get('prop:cols.internal.color')).toBe('white');
    expect(onChange).toHaveBeenCalledTimes(1);

    onChange.mockClear();
    onColUpdated.mockClear();
    model.props.cols.b = { color: 'blue' };
    expect(blockState.get('prop:cols.b.color')).toBe('blue');
    ...
    expect(blockState.get('prop:cols.a.color')).toBe('black');
    ...
    expect(blockState.get('prop:cols.a.color')).toBe('red');
    expect(blockState.get('prop:cols.internal.color')).toBe(undefined);
    expect(blockState.get('prop:cols.b.color')).toBe(undefined);
    expect(model.props.cols).toEqual({
      a: { color: 'red' },
    });
    expect(onColUpdated).toHaveBeenCalledTimes(1);
    ...
    expect(blockState.get('prop:cols.a.color')).toBe(undefined);
    ...
    expect((blockState.get('prop:title') as Y.Text).toJSON()).toBe('test');
    ...
    const getLabels = () => blockState.get('prop:labels') as Y.Array<unknown>;
    ...
    expect((blockState.get('prop:textCols.a') as Y.Text).toJSON()).toBe('test');
  ...
  test('stash and pop', () => {
    ...
    model.pop('cols');
    expect(blockState.get('prop:cols.a.color')).toBe('blue');
